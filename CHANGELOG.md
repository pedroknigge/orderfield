...
`of collect` names the legal home when a residual puts `docs_sync` (or `notes`, `tokens`) on the wrong object. `validate_schema` now also enforces the `anyOf` / `maxLength` / `patternProperties` keywords the public schemas already use. Successful `of close` wipes `work/scratch` so a CLOSED field does not keep media/`node_modules` bloat. Concurrent `of spawn` of the same child can no longer both pass the started-only check. `architecture.md` `MUTATING_COMMANDS` matches the real lock set (`gc` is inside). Same capability with less god-file: HITL `of issue` moves out of `scripts/of/cli/ops.py` into `scripts/of/cli/issue_cmd.py`. Status / resume / doctor stay on `ops`. Same 0.6 line. Not a new regime. `RUNTIME_OWNERSHIP` stays reserved. Do not rewrite v0.8.14 notes.
...
- **Proof:** `SchemaSubsetHonesty` (public schemas use only implemented keywords; empty `adapter_hints` fails `anyOf`; 257-char `denied_actions` item fails `maxLength`; `patternProperties` rejects a matching key; `validate_residual` / `validate_packet` refuse). C-030 extended. No new C-ID (honesty cap).
...
- **Proof:** `ClosedScratchWipe` (media/`node_modules` + wave logs gone; residual/CLOSE/ORDER stay; `--checklist` does not wipe; already-closed wipes leftover). `PostCloseTerminal` leftover PULSE is gone after close. `SkillClosedScratch`. C-084 extended. No new C-ID (honesty cap).
...
- **Proof:** `SpawnLockRace` (threaded claim: one ok / one refuse; two CLI `of spawn` of the same child: one starts). `MutatingCommandsHonesty` (docs quote `mutating_commands_prose()`; stale "gc is outside" gone). C-031 / C-058 extended. No new C-ID (honesty cap).
...
- **Proof:** `NoDuplicateCliDefs.test_ops_has_no_leftover_issue_defs` (ops has no leftover issue defs; `issue_cmd` owns them). `IssueConfirmLock` / `IssueSearchList` / `IssueCli` / `SkillIssueConfirm` stay green on the moved module. C-055 extended. No new C-ID (honesty cap). Relates to `#219` — not the whole god-module review.
