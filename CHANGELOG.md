...
- **Evaluator consent (Pedro):** at init / first wave ask once: "At the end, run fresh-context adversary + verifier (both)?" Store `--done-when-mission`. Do not pack/spawn review roles at start. Stored yes → before close pack+spawn both (not XOR). Stored no → contrast → `of close --checklist`. After close: project + OF `of learn` / `--promote`, not the review-role ask. `EvaluatorPacket.SPEAK_ASK` names the stored-end run. Not a new close gate. No new verb.
