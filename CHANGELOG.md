...
- **Reuse, not a supervisor:** `SpawnRecord.claim_started` takes `field.lock` only for the check+write. The write is live `dump_bytes` so WAL inherit does not delete a non-snapshot spawn rel. The child then runs unlocked. `register_spawned_child` uses `dump_bytes` under the existing registry flock (atomic replace, not `write_text`). `MUTATING_COMMANDS_ORDER` / `mutating_commands_prose()` is the lock-set source of truth. No new CLI / schema / process supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
