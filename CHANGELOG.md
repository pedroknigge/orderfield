...
=======
- **Reuse, not a new verb:** README lead is problem → what OF does → **When NOT to use OF** (InitAskSkip Small / stay-session). Front-page install is ≤3 glance lines (`ORDERFIELD_REF` + `--from-release` + PATH). Full SHA-256 ritual stays in PUBLISH / mortal-install. First close drops the unmeasured 30-second claim. Compose with #302 dest glance (orca/qwen rows) and #315 reserved honesty (hero omits `budget.tokens`; kernel says **not implemented**). No VERSION bump. `#307` / `#308`.
>>>>>>> 04da891 (docs: compose roast UX README with #315 reserved honesty)
...
- **Reuse table (design-first):** `SliceLint.long_note` already prints the ≥800 advisory. The remaining gap is that note claiming `The packet was still written` before pack can still die (`stale packets` / owns-path overlap) (`#267`).
- **Reuse, not a new verb:** emit the write-success clause only after the packet is on disk. Failed pack keeps the size advisory without the false write. No new CLI / schema / supervisor / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** appendix / troubleshooting keep the advisory; do not treat the size note as proof the packet exists.
- **Proof:** `SliceLintGate.test_failing_pack_does_not_claim_packet_written`. C-104 extended. No new C-ID (honesty cap). `#267`.
- **Reuse table (design-first):** `_cmd_spec_locked` already gated PAIR + `die()`. The remaining gap is `--verified-contract ID` without `--both-sides` looking like success while `verified_contract` does not move (`#268`).
- **Reuse, not a new verb:** same `die()` path as `--surface` without `--add`. PAIR without `--both-sides` exits nonzero, names `--both-sides`, leaves the seal count unchanged. `--both-sides` still seals. ContractSurface hide names id prefix or text cue. No new CLI / schema / supervisor / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** contrast / spec rows already teach PAIR `--both-sides`. The refuse is now unmistakable.
- **Proof:** `PairVerifiedContractRefuse`. C-047 extended. No new C-ID (honesty cap). `#268`.
...
- **Proof:** `HostMcpIsolate` / `PulseActivity.test_pulse_no_writes_yet_names_cpu` / `SkillHostMcp`. C-067 extended. No new C-ID (honesty cap). `#269`.
...
- **Proof:** `SpawnAdapterMissingGate` / `SkillSpawnAdapterMissing`. C-009 extended. No new C-ID (honesty cap). `#273`.
- **Reuse table (design-first):** `child_pulse_verdict` already returns `PACKED` without spawn/scratch. `resume_next_lines` already names `next SPAWN`. `SpawnRecord.present` / `live_pid` already exist. The remaining gap is pulse / status / resume printing `running` + residual MISSING for packed-never-spawned children (`#274`).
...
- **Reuse table (design-first):** `PlanCoverage` already extracts cited `##` / `###` IDs and WARNs orphans. `PlanDocSync.cited` already names `docs/plans/…`. The remaining gap is unused on-disk plans (chat `@folder` / paste never become field-owned) (`#312` / epic `#311` / `#278`).
- **Reuse, not a new verb:** init / first SPEC identity reads cited on-disk plan files or directories, pins `keep <rel> coverage honest` on constraints, and prints `plan_ingest N headings`. SPEC.md / `--source` paste / `@folder` is not ingest. Pack emits leftover IDs (`plan_cover bias` when the slice claims none). Constraint `plan_cover fail-closed` HOLDs `of close`. WARN default. No new CLI / schema / supervisor / `RUNTIME_OWNERSHIP`. No VERSION bump.
- **Skill drives the cut:** SKILL / appendix teach put the plan on disk, cite the path, high effort on ORDER; `@folder` is optional context only. Core stays under the 20KB cap.
- **Proof:** `PlanCoverageUnit` / `PlanIngestGate` / `SkillPlanFirstOrder` / `recovery/plan-first-coverage` / `recovery/plan-ingest-paste`. C-111 extended. No new C-ID (honesty cap). `#312`.
- **Reuse table (design-first):** ingest already maps `owns-requirement` ↔ heading (`PlanCoverage`). `PlanDocSync` already cites the user plan path. `OwnedWrite` / `CloseEvidence` already refuse false-green. The remaining gap is residual-done while the cited plan MD stays stale (`#313` / epic `#311` / `#278`).
- **Reuse, not a new verb:** `PlanWriteBack` surgically patches checkboxes / `Status:` / trailing `Shipped:` on collect of a green mapped heading. Fail-closed without OwnedWrite / close evidence. HITL if the plan is outside the project root or not cited. `docs_sync=done` only after plan bytes change or explicit skip. User plan path wins. No new CLI / schema / supervisor / `RUNTIME_OWNERSHIP`. No VERSION bump.
- **Skill drives the cut:** SKILL / appendix / CHILD teach write-back after green collect; do not invent completion; do not rewrite the whole plan. Core stays under the 20KB cap.
- **Proof:** `PlanWriteBackUnit` / `PlanWriteBackGate` / `SkillPlanWriteBack`. C-111 extended. No new C-ID (honesty cap). `#313`.
...
- **Proof:** `SkillWaveSettleAutoContinue` + `SkillSurface` pointers/markers lock the teaching. Packaging skill-size gate. No new C-ID (honesty cap). `#263`.
...
- **Reuse table (design-first):** `CloseChecklist` already owns close refuse. `SpawnRecord.flying` already names started-only over leftover residual. `validate_schema` already walks `additionalProperties` boolean. `ClosedScratch.wipe` already runs after a successful stamp. `refuse_child_forge` already gates pack/spawn/learn. `MissionRewriteRefused` already ignores residual replace keys. The remaining gap is five Medium dual-truths from the v0.8.16 pstack audit (`#233`): leftover residual + started-only still closed; session `pulse_verdicts` accepted `PACKED`; wipe-vs-stamp prose; `OF_CHILD` on patch/close/integrate; README slogan overstated append.
- **Reuse, not a new verb:** `CloseChecklist.flying` is `SpawnRecord.flying` for packed children. Object-form `additionalProperties` runs on the schema object (`PACKED` fails `pulse_verdicts`); `snapshot_session` stores only `ALIVE|QUIET|STALE`. close-is-proof says wipe after a successful stamp. `refuse_child_forge` on `of patch` / `of close` / `of integrate`. README: cannot replace mission/phase/constraint list; `constraints+` / `done_when+` append only on leader `integrate --apply`. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** SKILL / `/of` / appendix / README stay lockstep with the five Mediums. Core stays under the 20KB cap.
- **Proof:** `CloseChecklistFlying` (started-only + leftover residual refuses close). `SchemaSubsetHonesty.test_object_form_additional_properties_rejects_packed_pulse_verdict`. `ClosedScratchWipe` (scratch/spawns/prompts gone after stamp). `ChildForgeLeaderVerbs` (patch/close/integrate refuse `OF_CHILD`). `MissionRewriteRefused` (append only; replace ignored). C-030 / C-084 / C-119 / C-152 / C-025 extended. No new C-ID (honesty cap). `#234`.
- **Reuse table (design-first):** `validate_schema` / `validate_residual` / `SpawnResidual` / `StreamJson.residual` already own residual extract and the public-schema walk. `residual.codex.schema.json` already requires optional fields as `[type,null]` for structured output (`CodexStrictSchema`). The remaining gap is a generic `invalid residual extracted from stdout` one-liner when agy `--json-schema` / Codex `--output-schema` fills those nulls or emits a harness envelope (`#232`).
- **Reuse, not a loosened contract:** `CodexNullOmit` drops only public-schema-optional keys whose value is null (the inverse of `CodexStrictSchema.nullable`). `usage: null` stays legal. Extra keys (`error`, envelope slots) still fail and now name `$.path`. `SpawnResidual.payload` reuses `StreamJson.residual` unwrap (`structured_output` / `result` / `result_json` / `output`). No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** SKILL / `/of` / appendix teach Codex-null omit and `$.path` on invalid extract. Version lockstep only.
- **Proof:** `SpawnResidualExtract` (refuse line names `$.result_ref`; SUCCESS envelope names `$.status` + harness envelope; Codex-null `usage.tokens` / `v` lands). `ResidualSchemaContracts.test_codex_null_optional_fields_are_omit_not_a_new_contract` / `test_required_null_names_json_path`. Extra `error` still unexpected. No new C-ID (honesty cap). `#236`. Closes `#232`.
- **Reuse table (design-first):** `of learn --protocol` / `--promote`, PlanDocSync Mode A/B, and HITL `of issue` already own leftover destinations. DriveAfterIntegrate already says the report is not a stop. The remaining gap is wave-end leftovers dying as chat vapor (`#229`).
- **Reuse, not a new verb:** skill duty only. Ask promote / protocol / owned `docs/plans` / discard for leftover field `of learn`; defects → `of issue` HITL and/or PlanDocSync. Not auto-promote. Not a close gate. No `of triage`. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** SKILL table `after wave, before close` plus appendix `Wave-end / pre-close surplus`. SLAVE writes leftovers into residual/scratch so the ask has evidence.
- **Proof:** `SkillWaveEndTriage` (skill-surface unittest; appendix heading + not auto-promote). No new C-ID (honesty cap). `#229`.
- **Reuse table (design-first):** kernel `of eval` / `eval_cmd.py` / `evals/` recovery fixtures / CI `--strict --kernel` already exist. `SkillSurface` already splits core vs appendix. The remaining gap is teaching `of eval` as a field-run next (`#230`).
- **Reuse, not kernel deletion:** demote teaching only. SKILL / `/of` field-run table drop `eval`. Appendix `## Lab / eval` keeps `of eval --strict --kernel` and the demoted fixtures. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** field-run table only; lab proof points at the appendix. C-150 / C-153 stay honest.
- **Proof:** `FieldEvidenceHonesty` / `SkillSurfaceCore` (eval off hot path; still on appendix). `SkillWaveEndTriage` stays green. No new C-ID (honesty cap). `#230`.
- **Reuse table (design-first):** `CloseEvidence` already requires artifact SHA of `result_ref`. `EvaluatorPacket` already prints the review-packet row (not a close gate). `SkillPstackCherries` already teach prove-it ACCEPTANCE. `CloseChecklist` already forbids shipped without disk facts. The remaining gap is FACTIBLE / D CUMPLE from D prose while the published schedule overlaps, and XOR review-role consent at close (`#235`).
- **Reuse, not a new verb:** skill duty + `SkillArtifactProve` oracle (`scripts/skill_artifact_prove.py`). At init / first wave ask once: "At the end, run fresh-context adversary + verifier (both)?" Store `--done-when-mission`. Do not pack/spawn review roles at start. Stored yes → before close pack+spawn both (not XOR). Stored no → contrast → `of close --checklist`. After close: `of learn` / `--promote`, not the review-role ask. `EvaluatorPacket.SPEAK_ASK` names the stored-end run. No `of prove`. No hospital solver. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** before FACTIBLE / `of close` on schedule+invariants, a deterministic check on the **published** artifact (not memory, not D). Fail ⇒ INFACTIBLE or ROMPE. F covers the occupancy window. Core stays under the 20KB cap.
- **Proof:** `SkillArtifactProve` (teaching lock; CUMPLE-with-bed-overlap fixture fails; clean schedule and honest INFACTIBLE/ROMPE pass). `SkillEvaluatorPacket` (store-at-start; pack only if stored yes; XOR / start-Yes pack fail teaching). `EvaluatorPacketProof` stay not-a-close-gate. C-121 / C-151 / C-080 extended. No new C-ID (honesty cap). `#235`.
- **Reuse table (design-first):** `DriveAfterIntegrate` already prints `next` + speak after integrate when `in_flight=0` (#191). `cmd_spawn` still printed only `exit=` / `outcome=` and stopped (`#241`). The remaining gap is the post-child silent wait until a human says ok / `!of pulse`.
- **Reuse, not a new verb:** after spawn finalizes, `_emit_drive_after_spawn` calls `DriveAfterIntegrate.emit_from_disk` (same helper as integrate). Idle + residuals landed → `next COLLECT` + speak. Flying sibling / dry-run / generic handoff / `--json` stay quiet. Does not call collect. Does not hold `field.lock` for the child.
- **Proof:** `DriveAfterIntegrateProof.test_spawn_speak_collect_when_residual_landed` / `test_spawn_no_speak_while_sibling_flying`. `#241`. Relates `#239`.
- Packaging: VERSION 0.8.17; skill/alias description preview `v0.8.17 — …`. `install.sh` `DEFAULT_VERSION` in lockstep. Never rewrite v0.8.16 notes. These five cuts merged after the v0.8.16 tag.

## 0.8.16

Kernel work that landed on main after the published v0.8.15 tag (`b275c0ab`, #221+#227) while still claiming 0.8.15 lockstep: `validate_schema` now also enforces the `anyOf` / `maxLength` / `patternProperties` keywords the public schemas already use (#223). Successful `of close` wipes `work/scratch` so a CLOSED field does not keep media/`node_modules` bloat (#224). Concurrent `of spawn` of the same child can no longer both pass the started-only check; `architecture.md` `MUTATING_COMMANDS` matches the real lock set (`gc` is inside) (#225). Same capability with less god-file: HITL `of issue` moves out of `scripts/of/cli/ops.py` into `scripts/of/cli/issue_cmd.py` (#226; Relates #219). Status / resume / doctor stay on `ops`. Same 0.6 line. Not a new regime. `RUNTIME_OWNERSHIP` stays reserved. Do not rewrite v0.8.15 notes.

- **Reuse table (design-first):** `validate_schema` / `validate_public_schema` already own Draft 2020-12 subset checks (`type` / `const` / `enum` / `minLength` / `pattern` / `minimum` / `maximum` / `minItems` / `uniqueItems` / `items` / `required` / `additionalProperties` / `properties`). Packet `adapter_hints` already declares `anyOf` (tier or model). Residual `denied_actions` items already declare `maxLength` 256. State `path_index` already declares `patternProperties`. C-030 already claims runtime/public-schema parity. The remaining gap is those keywords slipping through, so invalid packets/residuals/state collect or the claim overstates.
- **Reuse, not a JSON Schema engine:** add `maxLength` next to `minLength`, `anyOf` as "at least one subschema matches", and `patternProperties` as "matching keys validate". `schema_unknown_keywords` walks public schemas so a future keyword cannot land as dual-truth. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`. Subtracting the keywords would shrink the claimed contract; enforcing the subset in use is the smaller honest fix.
- **Skill drives the cut:** SKILL / `/of` already teach public-schema enforcement. Version lockstep only.
- **Proof:** `SchemaSubsetHonesty` (public schemas use only implemented keywords; empty `adapter_hints` fails `anyOf`; 257-char `denied_actions` item fails `maxLength`; `patternProperties` rejects a matching key; `validate_residual` / `validate_packet` refuse). C-030 extended. No new C-ID (honesty cap). `#223`.
- **Reuse table (design-first):** SAT-002 already dumps closed-field scratch/logs/spawns/prompts on `of gc` (`closed-ephemeral`). `AuditPressure` already WARNs on fat scratch before close (not a gate). `_safe_unlink` already owns the delete. The remaining gap is the stamp leaving `work/scratch` until someone remembers `of gc`.
- **Reuse, not a new verb:** `ClosedScratch.wipe` runs after a successful stamp (and on `already spec_closed`) and unlinks this field's `work/scratch` children plus `waves/*/logs|spawns|prompts`. `--checklist` and refused close do not wipe. Contract files stay. Older ORDER fields stay workable; `of close` again or `of gc` cleans leftover scratch. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** SKILL / `/of` / appendix teach wipe-on-close next to the close-is-proof row. `of gc --audit` before close stays the pre-close WARN.
- **Proof:** `ClosedScratchWipe` (media/`node_modules` + wave logs gone; residual/CLOSE/ORDER stay; `--checklist` does not wipe; already-closed wipes leftover). `PostCloseTerminal` leftover PULSE is gone after close. `SkillClosedScratch`. C-084 extended. No new C-ID (honesty cap). `#224`.
- **Reuse table (design-first):** `field_lock` already serializes cooperating mutations. `cmd_spawn` already refuses a second started-only record and already takes `field.lock` to bump `children_spawned`. `dump_bytes` already writes live JSON without WAL capture. `MUTATING_COMMANDS` in `scripts/of/field.py` already includes `gc` (0.7.3). The remaining gap is a TOCTOU on `waves/<n>/spawns/<id>.json` (check then `dump_json` with no lock) and architecture/README/principles still listing the pre-`gc` set.
- **Reuse, not a supervisor:** `SpawnRecord.claim_started` takes `field.lock` only for the check+write. The write is live `dump_bytes` so WAL inherit does not delete a non-snapshot spawn rel. The child then runs unlocked. Spawn-registry persist stays `write_text` under the existing flock (Popen already started the child; `dump_bytes` fsync races LEARN-002 exec). `MUTATING_COMMANDS_ORDER` / `mutating_commands_prose()` is the lock-set source of truth. No new CLI / schema / process supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** `/of` and the appendix name the concurrent claim. Architecture durability paragraph quotes `mutating_commands_prose()`. Core SKILL stays under the byte cap.
- **Proof:** `SpawnLockRace` (threaded claim: one ok / one refuse; two CLI `of spawn` of the same child: one starts). `MutatingCommandsHonesty` (docs quote `mutating_commands_prose()`; stale "gc is outside" gone). C-031 / C-058 extended. No new C-ID (honesty cap). `#225`.
- **Reuse table (design-first):** `IssueConfirm` / `IssueList` / `_issue_scratch_rel_ok` already own HITL create, open-issue roster, and canonical `--body-file`. `eval_cmd.py` already proved the extract-by-existing-boundary pattern (`#219` first cut / `#222`). `field.py` stays the leftover I/O owner after SCOPE-GODSPLIT — inventing `field_lock.py` is named theater. The remaining gap is `ops.py` (~2975) mixing host ops with the HITL GitHub surface (`#219` second slice).
- **Reuse, not a rewrite:** move the existing issue block only. Public `of issue` argv, events, and `import of` names unchanged. No leftover `cmd_issue` / `IssueConfirm` / `IssueList` on `ops.py`. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
- **Skill drives the cut:** SKILL / `/of` still teach `--confirm` or TTY yes; `--dry-run` is not HITL; leader `ISSUE-*.md`. Appendix names `issue_cmd.py` as the HITL owner. SKILL stays at the 20KB cap.
- **Proof:** `NoDuplicateCliDefs.test_ops_has_no_leftover_issue_defs` (ops has no leftover issue defs; `issue_cmd` owns them). `IssueConfirmLock` / `IssueSearchList` / `IssueCli` / `SkillIssueConfirm` stay green on the moved module. C-055 extended. No new C-ID (honesty cap). Relates to `#219` — not the whole god-module review. `#226`.
- Packaging: VERSION 0.8.16; skill/alias description preview `v0.8.16 — …`. `install.sh` `DEFAULT_VERSION` in lockstep. Never rewrite v0.8.15 notes. These four cuts merged after the v0.8.15 tag.
...
- **Reuse, not a grok-only stack:** `StreamJson.ARGV["grok"]` is the documented `--output-format streaming-json` (not invented `stream-json`). `build_spawn_argv` places it before `-p`. Residual extract reuses `StreamJson` + stdout (`SpawnResidual`), including on timeout. `--json-schema` stays omitted (no documented file-path residual schema). Not a qwen-style residual no-op.
- **Finalize, not started-only:** timeout uses daemon reader threads and a bound join (`ChildIO`) so a leaked grandchild cannot hang finalize. `spawns/<id>.json` always gets `outcome` + `exit` + `ended_at` on exit, timeout, and missing binary. Same `finalize()` as every adapter.
- **Skill drives the cut:** SKILL / `/of` / appendix teach grok `streaming-json` and finalized spawn metadata in the same cut.
- **Proof:** `GrokAdapterSpawn` (stdout extract; missing binary / timeout finalize + timeout extract). `SpawnFinalization.test_timeout_finalizes_when_grandchild_keeps_stdout_open`. Existing `StreamJsonParse` / `SpawnFinalization` stay. No new CLI / supervisor / token ceiling.
- Packaging: VERSION 0.7.71; skill/alias description preview `v0.7.71 — …`. `install.sh` `DEFAULT_VERSION` in lockstep. Never rewrite v0.7.70 notes.
