...
- **Evaluator consent (Pedro):** ask both `--role adversary` and `--role verifier` at init / first wave; store on `--done-when-mission` / `--constraints-add`; run both at the configured end with no XOR re-ask. After close: project + OF `of learn` / `--promote`. `EvaluatorPacket.ASK_NEXT` / `SPEAK_ASK` name the stored both-roles run. Not a new close gate. No new verb.
