...
`of collect` names the legal home when a residual puts `docs_sync` (or `notes`, `tokens`) on the wrong object. `validate_schema` now also enforces the `anyOf` / `maxLength` / `patternProperties` keywords the public schemas already use. Successful `of close` wipes `work/scratch` so a CLOSED field does not keep media/`node_modules` bloat. Same 0.6 line. Not a new regime. `RUNTIME_OWNERSHIP` stays reserved. Do not rewrite v0.8.14 notes.
...
- Architecture: `MUTATING_COMMANDS` lists `gc` (code since 0.7.3 / SAT-006). Spawn lock race was not a verified bug — skipped.
