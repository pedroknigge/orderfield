...
- **Proof:** `SpawnAdapterMissingGate` / `SkillSpawnAdapterMissing`. No new C-ID (honesty cap). `#273`.
...
- **Proof:** `PackedOnlyNotAlive` (packed-without-spawn vs spawned-no-residual) / `SkillPackedOnlyStatus`. C-152 extended. No new C-ID (honesty cap). `#274`.
- Packaging: VERSION 0.8.21; skill/alias description preview `v0.8.21 — …`. `install.sh` `DEFAULT_VERSION` in lockstep. Never rewrite v0.8.20 notes. These two cuts merged after the v0.8.20 tag. `#269` / `#273` / `#274` land on the same line (no VERSION bump).
