...
- **Skill surface:** demote `eval` from SKILL / `/of` hot path (What to type next / alias default teach). Kernel `of eval` + tests stay. Appendix `#### Lab / eval` keeps the lab command honest. No VERSION bump (docs/skill-only; PackagingBump forbids docs-only tag).
- **Reuse table (design-first):** `CloseEvidence` already requires artifact SHA of `result_ref`. `EvaluatorPacket` already prints the review-packet row (not a close gate). `SkillPstackCherries` already teach prove-it ACCEPTANCE. `CloseChecklist` already forbids shipped without disk facts. The remaining gap is FACTIBLE / D CUMPLE from D prose while the published schedule overlaps (blind-test dual-truth).
- **Evaluator consent (Pedro):** ask both `--role adversary` and `--role verifier` at init / first wave (two packs / two children; neither wrote the slice). Store `--done-when-mission`. Default-propose both — not XOR. Legal ask: "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → `of close --checklist`. Opt-out is close, not one-role. After close: project + OF `of learn` / `--promote`. `EvaluatorPacket.ASK_NEXT` / `SPEAK_ASK` name the both-roles ask. Not a new close gate. No new verb.
- **Reuse, not a new verb:** skill duty + `SkillArtifactProve` oracle (`scripts/skill_artifact_prove.py`). Verifier/adversary read published artifact bytes. No `of prove`. No hospital solver. No new CLI / schema / supervisor / `of merge` / token ceiling / `RUNTIME_OWNERSHIP`.
...
- **Proof:** `SkillArtifactProve` (teaching lock; CUMPLE-with-bed-overlap fixture fails; clean schedule and honest INFACTIBLE/ROMPE pass). `SkillEvaluatorPacket` (both-roles ask; exclusive-or menus fail teaching). `EvaluatorPacketProof` stay not-a-close-gate. C-121 / C-151 / C-080 extended. No new C-ID (honesty cap).
...
- **Honesty (pstack Mediums 1–5, no VERSION bump):** `CloseChecklist.flying` is `SpawnRecord.flying` for packed children (leftover residual + started-only refuses `of close`). `validate_schema` runs object-form `additionalProperties` (`PACKED` fails session `pulse_verdicts`); `snapshot_session` stores only `ALIVE|QUIET|STALE`. close-is-proof says wipe after a successful stamp; `ClosedScratchWipe` asserts scratch/spawns/prompts gone. `refuse_child_forge` on `of patch` / `of close` / `of integrate`. README slogan: cannot replace mission/phase/constraint list; `constraints+` / `done_when+` append only on leader `integrate --apply`. **Proof:** `CloseChecklistFlying`; `SchemaSubsetHonesty.test_object_form_additional_properties_rejects_packed_pulse_verdict`; `ClosedScratchWipe`; `ChildForgeLeaderVerbs`; `MissionRewriteRefused`.
