...
- **Reuse table (design-first):** `CloseEvidence` already requires artifact SHA of `result_ref`. `EvaluatorPacket` already asks adversary/verifier before close. `SkillPstackCherries` already teach prove-it ACCEPTANCE. `CloseChecklist` already forbids shipped without disk facts. The remaining gap is FACTIBLE / D CUMPLE from D prose while the published schedule overlaps (blind-test dual-truth).
...
- **Proof:** `SkillArtifactProve` (teaching lock; CUMPLE-with-bed-overlap fixture fails; clean schedule and honest INFACTIBLE/ROMPE pass). C-121 / C-151 extended. No new C-ID (honesty cap).
