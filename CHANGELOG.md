...
- Packaging: VERSION 0.8.22; skill/alias description preview `v0.8.22 — …`. `install.sh` `DEFAULT_VERSION` in lockstep. Never rewrite v0.8.21 notes. These five cuts merged after the v0.8.21 tag (`#267`–`#274` / PRs `#270`–`#276`). Plan-first (`#279`), CloseEvidence (`#286`), write-floor (`#294`), skill load-tax (`#287`), evaluator consent (`#288`), HITL.md (`#290`), and InitAskSkip (`#289`) landed on the same heading after that lockstep.
