...
- **Spawn environment allowlist:** children no longer inherit the parent environment (proxy / CA / SSH-agent / Windows base vars kept; `OF_FIELD` always set). `OF_SPAWN_ENV=NAME1,NAME2` extends the allowlist; `OF_SPAWN_ENV=inherit` opts out. Children get no stdin. Spawn metadata is finalized on every outcome (exit, timeout, missing binary, interrupt, OS error); the post-run `children_spawned` bump re-reads state under the field lock.
