...
`of collect` names the legal home when a residual puts `docs_sync` (or `notes`, `tokens`) on the wrong object. `validate_schema` now also enforces the `anyOf` / `maxLength` / `patternProperties` keywords the public schemas already use. Successful `of close` wipes `work/scratch` so a CLOSED field does not keep media/`node_modules` bloat. Concurrent `of spawn` of the same child can no longer both pass the started-only check. `architecture.md` `MUTATING_COMMANDS` matches the real lock set (`gc` is inside). Same 0.6 line. Not a new regime. `RUNTIME_OWNERSHIP` stays reserved. Do not rewrite v0.8.14 notes.
