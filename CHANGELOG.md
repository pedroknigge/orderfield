...
- **Finalize, not started-only:** timeout closes the captured pipes (`ChildIO`) so a leaked grandchild cannot hang the reader join. `spawns/<id>.json` always gets `outcome` + `exit` + `ended_at` on exit, timeout, and missing binary. Same `finalize()` as every adapter.
