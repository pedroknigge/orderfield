...
16. **Transitions prove closure.** `next-wave` requires no in-flight children, a complete current-digest report, and a post-escalation ORDER revision when blocked. `phase` additionally requires the current phase closed, the next official phase, and report regime `phase`. `phase --force --reason` records an audited override; it is the only transition break-glass surface. Deliver additionally requires binding requirements to be owned and verified, `of close` (`spec_closed`), and SPEC.md bytes matching `ORDER.spec_hash`.
17. **ORDER may compress reasoning, never the contract.** The current brief lives at `.orderfield/SPEC.md` (`spec_ref` + `spec_hash`): the original request plus dated amendments. Do not write `PROMPT.md` at the project root — ingest is disposable after copy. A new human request is `of spec --amend` (IDs continue); a dropped requirement is `--supersede`; a full replace is `--revise-file` and archives to `spec-log` (episodic, 30 days). Silent rewrite is a field error. Packets fit on one screen, reference-load SPEC, and own stable requirement IDs. Slice `done` is local; SPEC `closed` is `of contrast` then `of close`.
