...
16. **Transitions prove closure.** `next-wave` requires no in-flight children, a complete current-digest report, and a post-escalation ORDER revision when blocked. `phase` additionally requires the current phase closed, the next official phase, and report regime `phase`. `phase --force --reason` records an audited override; it is the only transition break-glass surface. Deliver additionally requires binding requirements to be owned and verified (`of spec` / `of spec-diff`).
17. **ORDER may compress reasoning, never the contract.** The verbatim user brief lives at `.orderfield/SPEC.md` (`spec_hash`). Packets fit on one screen and reference-load SPEC; they do not replace it. The verifier compares SPEC ↔ ORDER ↔ product. A compressed mission that omits CLI, types, exit codes, or deliverables is a field error, not a successful reduction of degrees of freedom.
