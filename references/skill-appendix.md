...
2. `of init --mission "…" --source "<verbatim brief>" --campo` opens `.orderfield/campo/` on this git branch and this cwd. It does not create a worktree. It does not appoint a leader. Contestant #1 is the session running `of` (first roster peer must be that harness). It competes; it is not a parent. `of` writes `campo/spawns.json` with headless argv for #2..N (`--model` via the existing adapter). This tracer records that argv and does not launch it (`launched` false). Same cwd and branch.
