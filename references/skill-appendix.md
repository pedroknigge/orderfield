...
**`--owns-path` must cover every path `--slice` names.** Tests and components count. Pack WARNs `owns_path_incomplete` when a slash-containing slice path sits outside the write set (packet is still written). Implementer with empty `--owns-path` WARNs `owns_path_empty`. Do not pack an incomplete write set and hope the child invents the rest — that is escalate-or-zero-writes theater. `of unpack --child-id <id>` then re-pack with `--owns-path` (repeatable). Distinct from collect-time zero owned writes (`OwnedWrite` / #251 / #252). Parallel ark-check of sibling WIP is project-harness noise — not an OF cut.
