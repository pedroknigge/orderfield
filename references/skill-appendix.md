...
| Large: colliding / public-claim / multi-role / multi-harness | consult + **must propose** once/field | **must ask** once/field | **must ask** once; store `--done-when-mission`; never silent. Stored yes → pack+spawn both. Stored no → contrast → close |
...
- Do not close on self-praise. **InitAskSkip** Large: at init / first wave **must ask** once (store `--done-when-mission`; do not pack/spawn): "At the end, run fresh-context adversary + verifier (both)?" Default both. Stored yes → before close pack+spawn both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children; neither wrote the slice). Stored no → contrast → `of close --checklist`. InitAskSkip Small: skip the store-ask; contrast → close. After close, ask `of learn` / `--promote`, not the review-role ask. Never silent on Large. Not a new close gate. Not after ordinary integrate/next-wave. Not `of merge`.
