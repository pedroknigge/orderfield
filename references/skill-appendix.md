...
Product surface: [README.md](../README.md). Child contract: [SLAVE.md](../SLAVE.md). Invariants: [principles.md](principles.md). Adapters: [adapters.md](adapters.md).
...
- Do not claim shipped / closed / done on a field without running `of contrast` and `of close --checklist` in the same turn and quoting `contrast RESOLVED` plus `residual empty`. If either fails, you may not claim shipped. Mechanical — not your judgment. The checklist prints `speak`. Pair with quote-PULSE while residual is MISSING.
