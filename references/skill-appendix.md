...
| Large: colliding / public-claim / multi-role / multi-harness | consult + **must propose** once/field | **must ask** once/field | **must ask** once; store `--evaluator-consent`; never silent. Stored yes → after each wave settle pack+spawn both on that wave residual before next-wave. Stored no → skip review; contrast → close |
