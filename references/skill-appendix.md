...
Campo is the default for `of init` / `of new`. The roster question to the user is "which models compete in Campo?" with the stored roster (`of config`) as the default answer. One harness is not plain Orden: a single CLI with two or more catalog models still seats a Campo (for example claude haiku / sonnet / opus). `--multi-model no` is a budget hint, never an Orden switch. Only fewer than two distinct (cli, model) seats degrade to Orden. Pass `--orden-only=user --orden-reason "<the user's words>"` only when the user explicitly asks for plain Orden; the kernel records it in `ORDER.orden_only` and refuses a bare `--orden-only` while a roster is stored.

1. `of config` audits installed harness CLIs (PATH≠auth) and the catalog models on those CLIs. `of config set --contestant MODEL EFFORT` (repeat, N≥2) stores the contestants list (`OF_CONFIG` or `~/.orderfield/config.json`). Each seat is `{model, effort}` only. Peers: list order is not rank and not a role. The file stays empty until set. There is no leader key. Example data only (not a default, not a rank): Opus 5.5 / medium, Gemini 3.8 Flash / medium, Codex Sol 6 / high, Grok 4.7 / high. Choose a name only when `of config` lists it as installed. `Campo.resolve_spawn` binds that model to the one installed harness, and refuses a missing or ambiguous name.
2. `of init --mission "…" --source "<verbatim brief>" --campo` opens `.orderfield/campo/` on this git branch and this cwd and launches peers #2..N headless. It does not create a worktree. It does not appoint a leader. Contestant #1 is the session running `of` (first roster peer must be that harness). It competes; it is not a parent. `campo/spawns.json` records the adapter argv, pid, exit code, and exit, dead, or timeout, plus a stderr tail when the child wrote one. The child PATH keeps the leader entries first and gains `/bin` and `/usr/bin` when those are missing (macOS keeps mkdir and cat in `/bin`, next to a git that lives in `/usr/bin`). Same cwd and branch. A missing harness CLI refuses before any peer is launched.
3. This session writes `campo/proposals/c1.md` and `campo/ballots/c1.json` (`claim`, `evidence`, `peer`, `stance` of `concede` or `challenge`). The kernel writes nothing for c1. Headless peers write their own proposal and ballot.
4. The kernel waits until every proposal and ballot is valid, until every headless peer has exited, or until the max deadline (600s code default; `of config set --deadline` stores `deadline_s`; `OF_CAMPO_DEADLINE` overrides). All valid ballots pin immediately—do not wait out the ceiling. When peers exit early without a full ballot set, settle with what is present (quorum may hold or write `campo/hold.json`). At the max deadline, quorum is a strict majority that includes c1 and at least one concede (N=2 needs both; N=3 needs 2 including c1). That pins with the ballots present. Otherwise `campo/hold.json` and a HITL line for the leader: write the missing `campo/ballots/<id>.json`, then run `of campo settle`. Do not ask the human to type a command. Plurality of concedes wins. A tie breaks by contestant id. No concede writes no `leader.json`. `of campo settle` is that retry and still requires every ballot.
...
| Deep module: simple caller interface, complexity hidden; design-twice when two shapes | Hurry-to-green / lights-off / shallow interface; unittest/CI green as Ready |
...
| Large: colliding / public-claim / multi-role / multi-harness | consult + **must propose** once/field | **must ask** once/field | **must ask** once; store `--evaluator-consent`; never silent. Stored yes → after each wave settle pack+spawn both on that wave residual before next-wave. Stored no → skip review; contrast → close; execute printed next |
...
`ORDER.agent_band.band` is optional (#304). Unset infers from this-wave non-review count. Green scoped review still executes printed `next`. Init-ask-skip does not name these bands. Large non-trivial fork: design-twice two approaches before pack. Unittest/CI green is not Ready.
...
Serious multi-agent work and long missions use the **full verb table** in the core — resume, pack, spawn/handoff, collect, integrate, contrast, close, patch, next-wave, `of spec --amend`. That is the field-run table. Lab `eval` is appendix, not a production next-step. Do not invent a process supervisor, bot org, `RUNTIME_OWNERSHIP` telemetry, fake token budgets (`of pack --tokens` N>0 dies), or `of merge` (parent close returns ACTIVE). Walk: [docs/long-mission.md](../docs/long-mission.md). Token budgets / `scale_up` / `scale_across` are **not implemented** (`scripts/of/regime.py`).
...
| No invented runtime | Forbidden + leftovers **not implemented** | supervisor, bot org, fake token budgets, `of merge` |
...
**Plan-first ORDER.** Classify ingress **folder | chat | prompt** (`plan_ingress` cue), then enough? gather/ask gaps (pstack cherry bias) — only then write ORDER. Materialize+fidelity **before pack** (`PlanIngress`). folder: cite existing `docs/plans/…` only; kernel must not invent a parallel plan MD. chat: write a durable capture first (`docs/plans/<owner>/chat-capture-<ts>.md` or `.orderfield/plan-source.md`); missing → speak next, no silent thin SPEC. prompt: ultra-detailed `--source` / `ingest.md` / `prompt.md` (needles: PROHIBIDO / DoD / COLA / FASE 0) **is** the plan — promote **verbatim**, pin `plan_source <rel> sha=`, then discard disposable. Improve = explicit amend only. Paste without promote stays honest (`plan_cover paste`; chat paste / `@folder` is not ingest). `plan_fidelity gap|invent` HOLDs pack (default ON once prompt/chat source is pinned). `invent` is a plan MD **created after pin baseline** (`.orderfield/plan-baseline.json`), not an unpinned pre-existing file in a shared `docs/plans` tree. Digest every requirement section (`## AUTH-001 …`) into a wave + `of pack --owns-requirement` + `--owns-path` (uncovered first). Tracer-bullet wave 1. high effort on ORDER only; children **medium** (do not re-architect). `plan_cover orphan` WARN; `plan_cover fail-closed` HOLDs close. Not a new verb. ORDER bias (not essays): architect · sequence-verifiable-units · encode-lessons-in-structure · blast-radius · prove-it-works · attack-the-premise. Proof: `of eval recovery/plan-first-coverage` / `recovery/plan-ingest-paste` / `recovery/plan-ingress-promote` / `recovery/plan-ingress-fidelity`.

**Plan write-back.** After a wave **collects green** on an owns-requirement mapped to a cited heading, surgically edit that user plan MD (`PlanWriteBack`): checkbox `[ ]`→`[x]`, `Status:` line, trailing `Shipped: wave N · PR/SHA`. Do not invent completion without OwnedWrite / close evidence. Do not rewrite the whole plan. HITL if the plan is outside the project root or not cited. Residual `docs_sync=done` only after the plan file bytes actually changed, or explicit skip (`plan_write skip` / `docs_sync skip` in `proposed_patch.notes`). User plan path wins — `.orderfield/` is not a second documentation system. Proof: `PlanWriteBackUnit` / `PlanWriteBackGate`.

**Wave-end / pre-close surplus.** After close (not between waves; not before next-wave). `DriveAfterIntegrate` / `resume_next_lines` already own settle when `in_flight=0` + printed `next`. Stored yes (`EvaluatorPacket.consent_of` / `ORDER.evaluator_consent`): after each implementer wave settle pack+spawn both `--role adversary` and `--role verifier` on that wave's published residual before next-wave (`due` → printed `PACK`; verifier reads bytes — `OwnedWrite` / `SkillArtifactProve`, not implementer prose). Review refuse → HOLD (`review refused; do not next-wave`). Review green / stored no → skip review and execute printed next. Leftover field `of learn` notes and reportable errors are a HITL **must ask after close** — do not let them die with the field or stay chat vapor. EvaluatorPacket consent stays `ORDER.evaluator_consent` via `of patch --evaluator-consent` (store-at-start). Do not invent a stop on the settle path. Not auto-promote-all. Not a close gate. Not a new verb. Not a process supervisor. Doctor `docs_sync` / `AuditPressure` stay advisory; this duty is skill, not a kernel auto-promote.
...
**Slice fields (packet template).** `--slice` / handoff / CHILD carry the brief — not an orch inbox. CONTEXT is file/SPEC pointers, never the parent transcript.
...
| TIMEBOX | `budget.seconds` only. `budget.tokens` **not implemented** (`--tokens` dies) |
| FORBIDDEN | ORDER.constraints + packet `workspace.forbidden` |
| REPORT | residual shape in CHILD (`status` / `result_ref` / `residual` / `metrics`) |
...
**Disjoint `--owns-path` is not enough.** `--owns-path` is a file write-set. A git worktree has one HEAD and one index, shared by every process in it. Two implementers with disjoint files still overwrite each other's branch and staged files. Pack refuses `shared_worktree` when a second `--role implementer` is packed and no `of worktree add` is recorded for the unsheltered children (`of pack --force` shares one worktree anyway). Isolate with `of worktree add --child-id` for each, or spawn them in series (collect the first before the second starts). Same-wave overlapping `--owns-path` still dies. A second implementer in the wave **must** pass `--owns-path`. Cross-wave reuse of a path prints a note (`consider continuing <child>`) — not a lock. If the next work is the same files, that child is in-flight: continue from scratch; do not pack a sibling.
...
**Agent-band + multi-model (once per field).** At init / first wave **must ask** once (task-dependent; store `of patch --agent-band 1-4|5-10|10-50` and `--multi-model yes|no`, or pass the same flags on `of init`). Do not re-ask each wave (breaks auto-continue). The band is a **wave budget**, not a spawn cap and not `caps.max_children`. Task + adapters are the ceiling, not OF. Children default **medium** effort on implementer slices (`adapter_hints.effort`; high is ORDER authoring, not a child slice). `of doctor` / `of init` print `ram_total_gb` (+ optional avail) and a suggested band from stdlib host RAM (Linux `/proc/meminfo`, macOS `sysctl hw.memsize`, Windows `GlobalMemoryStatusEx`). Leader host RAM is not a cloud/remote worker — confirm; do not silently size a VM. Optional low-avail note is WARN only, never a spawn refuse. Large-band scoped review is #282 (not this cut). No new verb. No hard-cap at 50.

**Per-task model hints (leader initiative).** **InitAskSkip** Large: when you understand a real multi-slice / multi-role wave (once per field: init / first pack), **consult** [docs/model-catalog.md](docs/model-catalog.md) (or `docs/model-catalog.json`) first, then **must propose** a distribution in chat before packing. Example: "¿Distribuyo explorer/boilerplate/synthesizer en cheap y implementer/adversary/verifier/threshold en frontier?" Use the catalog's list prices and notes — do not assume smarter = more tokens or more expensive. On yes, write it — do not invent a model, do not silently switch. InitAskSkip Small (1-2 exclusive slices; bump / obvious) is not this beat. Never silent switch. Token budgets are **not implemented**.
...
Oversized residual speak is a handle (`ObservationPack`; ≥10KiB): `handle <path> <size>B` plus `head`/`tail` excerpts. Full bytes stay on disk. Do not paste the body into chat. Read the path with existing tools (no new verb). Do not strip `OF_EVIDENCE_RECEIPT` / `evidence_receipt` / `---RECEIPT---` markers (#284). After `in_flight=0`, cite ORDER + handles — do not re-ingest prior waves' full residuals. Wave-end verifier/adversary (#303 / #280) read those paths (and receipts), not pasted blobs. Small residuals may still print fully. Proof: `of eval recovery/observation-pack`.
...
`status=done` close evidence is not a caption. Collect (`CloseEvidence`) requires `artifact_sha:` — the sha256 of the proof file — and `rollback:` a verb command (`git`/`rm`/`of`/…), not a filename. Explorer / adversary / verifier without `--owns-path` may hash `result_ref` (scratch notes are fine). Implementer / `--owns-path` must hash owned product bytes or a named published artifact, not `.orderfield/work/scratch/`. A slogan (`we hashed it`, `rollback: revert the change`, `rollback: notes.md`) or a hash that does not match the proof file dies. Threshold/blocked skip the gate. Slice `done` is still not SPEC closed. Do not invent a second close doctrine or a new residual key.

Build/test (and similar) output above the size floor is an **evidence receipt**, not a second close doctrine. Archive the exact bytes under scratch, then cite `evidence_receipt: <sidecar>` with source hash, exit, key quotes/paths, size, and command id (`EvidenceReceipt`; SoL-Pi §2.4). Emit may FALLBACK to the original archive (no size win / cannot compact). A cited receipt that fails verify is collect INVALID — bad receipt ≠ green. FALLBACK is not collect-green. File reads and search results bypass the reducer; do not summarize away source the child owns. Optional cheap extract stays behind the gate. Wave-end verifier (#280 / PR #303) prefers receipts + published artifacts over implementer narrative. ObservationPack (#283) must not strip a verified `evidence_receipt:` / `OF_EVIDENCE_RECEIPT` line. Not trust-the-summary. Not Action Fusion. Design: [docs/design/evidence-receipt.md](../docs/design/evidence-receipt.md).
...
Regimes: `escalate_up | scale_out | scale_across | scale_up | human | hold | phase`. `scale_across` and `scale_up` are **not implemented** (legacy enums; remapped to hold). Packet `budget.tokens`, `thresholds.local_budget_pct`, and inherited depth are **not implemented** — schema leftover `tokens=0`; `--tokens N` for N>0 dies; only `budget.seconds` is enforced, as the spawn wall-clock. `of spawn --timeout` must match packet seconds or be omitted. The kernel does not invent telemetry.
...
- Do not treat unittest or CI green as Ready. Ready is named interface simplification + proof. Non-trivial fork: design-twice. Prefer a deep module. No new verb.
- Do not close on self-praise. **InitAskSkip** Large: at init / first wave **must ask** once (`of patch --evaluator-consent yes|no`; do not pack/spawn): "At the end, run fresh-context adversary + verifier (both)?" Default both. `ORDER.evaluator_consent` is the disk key (#280 reads it). Stored yes → after each wave settle pack+spawn both `--role adversary` and `--role verifier` on that wave residual before next-wave (fresh-context review packet; two packs / two children; neither wrote the slice). Stored no → skip review; contrast → `of close --checklist`; execute printed next. Review refuse → HOLD. Missing key → evaluator unset (not ask). InitAskSkip Small: skip the store-ask; contrast → close. After close, ask `of learn` / `--promote`, not the review-role ask. Never silent on Large. Not a new close gate. Not a second ask after ordinary integrate/next-wave. Not `of merge`.
