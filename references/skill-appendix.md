...
4. The kernel waits until every ballot is valid, or until the deadline (120s, `OF_CAMPO_DEADLINE`). All valid ballots pin immediately. At the deadline, quorum is a strict majority that includes c1 and at least one concede (N=2 needs both; N=3 needs 2 including c1). That pins with the ballots present. Otherwise `campo/hold.json` and a HITL line: write the missing `campo/ballots/<id>.json`, then `of campo settle`. Plurality of concedes wins. A tie breaks by contestant id. No concede writes no `leader.json`. `of campo settle` is that retry and still requires every ballot.
