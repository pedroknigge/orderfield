...
- Do not close on self-praise. Ask both `--role adversary` and `--role verifier` at init / first wave; store the choice; run both at the configured end. Do not re-ask a XOR menu. After close, ask `of learn` / `--promote`. Never silent at start. Not a new close gate. Not after ordinary integrate/next-wave. Not `of merge`.
...
Use the minimum. Explorer + adversary already prove the principle. A fresh-context `adversary` and `verifier` pair is the independent review (council shape). **Must ask** both at init / first wave; store; run at end; do not re-ask. Implementer self-praise is not review. Not a new close gate.
