...
**Per-task model hints (leader initiative).** When you understand a real multi-slice / multi-role wave (init → first pack, or when re-planning roles), **consult** [docs/model-catalog.md](docs/model-catalog.md) (or `docs/model-catalog.json`) first, then **must propose** a distribution in chat before packing. Example: "¿Distribuyo explorer/boilerplate/synthesizer en cheap y implementer/adversary/verifier/threshold en frontier?" Use the catalog's list prices and notes — do not assume smarter = more tokens or more expensive. On yes, write it — do not invent a model, do not silently switch. A single obvious pack is not this beat. Never silent switch. Never `of pack --tokens`.
...
**Wave harness plan (leader initiative).** After you understand the mission (and after the cheap vs frontier propose when it applies), **consult** the catalog, then **must ask** once in chat before packing: same-harness roles on one harness vs a multi-harness mix. Playbook below. A single obvious pack on the current harness is not this beat. Never silent mix. Never invent adapters. Not a router.
...
- Do not end a turn with a status report after collect+integrate while `next` is work (`NEXT-WAVE` / `PACK` / `COLLECT` / …). Report is not a stop. Do not wait for ok/pulse. HOLD is continue packets, not invent-consent. Ordinary next-wave/pack is not the adversary/harness/model ask.
