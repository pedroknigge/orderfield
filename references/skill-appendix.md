...
**Wave-end / pre-close surplus.** After collect+integrate (`DriveAfterIntegrate`) and before `of close` / `--checklist`, do not let leftovers die with the field or stay chat vapor. **Must ask** (HITL). Same turn as printed `next` — do not skip drive; do not invent a stop. Not auto-promote-all. Not a close gate. Not a new verb. Not a process supervisor. Doctor `docs_sync` / `AuditPressure` stay advisory; this duty is skill, not a kernel auto-promote.
...
**Per-task model hints (leader initiative).** When you understand a real multi-slice / multi-role wave (once per field: init / first pack), **consult** [docs/model-catalog.md](docs/model-catalog.md) (or `docs/model-catalog.json`) first, then **must propose** a distribution in chat before packing. Example: "¿Distribuyo explorer/boilerplate/synthesizer en cheap y implementer/adversary/verifier/threshold en frontier?" Use the catalog's list prices and notes — do not assume smarter = more tokens or more expensive. On yes, write it — do not invent a model, do not silently switch. A single obvious pack is not this beat. Never silent switch. Never `of pack --tokens`.
...
**Wave harness plan (leader initiative).** After you understand the mission (and after the cheap vs frontier propose when it applies), **consult** the catalog, then **must ask** once per field in chat before first pack: same-harness roles on one harness vs a multi-harness mix. Later waves: only on efficiency propose or an explicit harness change. Playbook below. A single obvious pack on the current harness is not this beat. Never silent mix. Never invent adapters. Not a router.
...
Read-only activity heuristic over the in-flight children: when any child is flying it prints `running` (residual MISSING; harness chrome is not the field), then per child when it was packed, the newest write in its scratch, the last 1–3 `PULSE` progress lines, and the newest shared-repo product write (`.orderfield/` excluded), then a verdict — `PACKED` (no spawn record and no scratch writes), `ALIVE` (open spawn: no `ended_at` / outcome, plus recent spawn/scratch evidence), `QUIET` (< 30 min, normal during long installs/tests), `STALE` (`--stale-min` overrides), `done_without_residual` (settled spawn without a schema-valid residual — leftover PULSE mtime is not ALIVE). Packed-only is not ALIVE. A harness `result success` that wrote no residual is `outcome=done_without_residual`, not a healthy `ok`. Host `denied_actions` Write are not `escalate_up` / tool_failures — salvage then collect; do not PATCH THEN NEXT-WAVE as if the slice failed. Proof: `recovery/spawn-ended-without-residual`. A missing or empty `PULSE` stays `running` — do not invent a crash. Scratch includes the child's contract-required heartbeat, and the repo signal is shared across children, so pulse is neither process health nor per-child product-write attribution. `STALE` is a signal, not an action: the kernel never kills or unpacks; releasing a dead child stays a human/leader call (`of unpack`). Pulse does not mutate ORDER, state, session, or wave artifacts; its update-notice throttle may write the user cache (`~/.cache/orderfield/update-check.json`, or `OF_UPDATE_CACHE`). Do not use pulse as a checkpoint.

**Stay-on-the-run.** Pulse `STALE` means continue the **same packet this turn** (`of handoff` or `of spawn` on the existing packet). Do not unpack by default. Do not wait forever. Do not pack a sibling. `of pulse --watch` exits when idle (prints next; do not sleep); it is not a daemon, not a 5-minute kernel loop, and not a process supervisor. A harness `/loop` may keep a chat alive outside Orderfield; it is not an OF process supervisor. The kernel still never kills or unpacks on STALE — a truly dead child is an explicit `of unpack`.
...
`status=done` close evidence is not a caption. Collect (`CloseEvidence`) requires `artifact_sha:` — the sha256 of `result_ref` bytes — and `rollback:` a command that can undo the artifact (a path or `git`/`rm`/`of`/…). A slogan (`we hashed it`, `rollback: revert the change`) or a hash that does not match the file dies. Threshold/blocked skip the gate. Slice `done` is still not SPEC closed. Do not invent a second close doctrine or a new residual key.

`status=done` / success is not proof of product work. Collect (`OwnedWrite`) also requires at least one meaningful write under `--owns-path` or a recorded worktree since spawn when the role is implementer or `owns_paths` is set. Zero owned files + `status=done` is `owned_write_missing` — INVALID, not green. Explorer / adversary / verifier may touch zero product files unless they took `--owns-path`. Do not CUMPLE-wash. Do not trust status alone. Not `of prove`. Not a supervisor.
...
- Do not end a turn with a status report after collect+integrate while `next` is work (`NEXT-WAVE` / `PACK` / `COLLECT` / `INTEGRATE` / …). Report is not a stop. Do not wait for ok/pulse. HOLD is continue packets, not invent-consent. Ordinary next-wave/pack is not the adversary/harness/model ask.
...
- Do not close on self-praise. At init / first wave **must ask** once (store `--done-when-mission`; do not pack/spawn): "At the end, run fresh-context adversary + verifier (both)?" Default both. Stored yes → before close pack+spawn both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children; neither wrote the slice). Stored no → contrast → `of close --checklist`. After close, ask `of learn` / `--promote`, not the review-role ask. Never silent at start. Not a new close gate. Not after ordinary integrate/next-wave. Not `of merge`.
...
Use the minimum. Explorer + adversary already prove the principle. A fresh-context `adversary` and `verifier` pair is the independent review (council shape; two packs / two children; neither wrote the slice). **Must ask** once at init / first wave (do not pack/spawn): "At the end, run fresh-context adversary + verifier (both)?" Stored yes → before close pack+spawn both. Stored no → contrast → close. After close: `of learn`, not the review-role ask. Implementer self-praise is not review. Not a new close gate.
