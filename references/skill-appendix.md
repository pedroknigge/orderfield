...
Build/test (and similar) output above the size floor is an **evidence receipt**, not a second close doctrine. Archive the exact bytes under scratch, then cite `evidence_receipt: <sidecar>` with source hash, exit, key quotes/paths, size, and command id (`EvidenceReceipt`; SoL-Pi §2.4). The deterministic gate accepts or falls back to the original (tampered quote/hash/exit, credential suspicion, no size win). A cited receipt that fails verify is collect INVALID — bad receipt ≠ green. File reads and search results bypass the reducer; do not summarize away source the child owns. Optional cheap extract stays behind the gate. Wave-end verifier (#280 / PR #303) prefers receipts + published artifacts over implementer narrative. ObservationPack (#283) must not strip a verified `evidence_receipt:` / `OF_EVIDENCE_RECEIPT` line. Not trust-the-summary. Not Action Fusion. Design: [docs/design/evidence-receipt.md](../docs/design/evidence-receipt.md).
