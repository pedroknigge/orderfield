...
- Do not close on self-praise. Ask both `--role adversary` and `--role verifier` at init / first wave (fresh-context review packet; two packs / two children; neither wrote the slice). Store `--done-when-mission`. Default-propose both. Ask: "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → `of close --checklist`. Opt-out is close, not one-role. After close, ask `of learn` / `--promote`. Never silent at start. Not a new close gate. Not after ordinary integrate/next-wave. Not `of merge`.
...
Use the minimum. Explorer + adversary already prove the principle. A fresh-context `adversary` and `verifier` pair is the independent review (council shape; two packs / two children; neither wrote the slice). **Must ask** both at init / first wave. "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → close. Opt-out is close, not one-role. Implementer self-praise is not review. Not a new close gate.
