...
| A multi-slice field is about to start | **InitAskSkip** Large: at init / first wave **must ask** once: "At the end, run fresh-context adversary + verifier (both)?" Default both. Store on `done_when`. Do not pack/spawn. Stored yes → before close pack+spawn both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children; neither wrote the slice). Never silent. Stored no → contrast → `of close --checklist`. InitAskSkip Small (1-2 exclusive slices; bump / obvious): skip the four theater asks; contrast still required. After close, ask `of learn` (project + OF), not the review-role ask. Self-praise is not review. Not a new close gate. |
