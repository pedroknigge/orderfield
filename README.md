...
<p align="center">
  <strong>v0.8.16</strong> · contract kernel · MIT · Python 3.11+ stdlib · <a href="https://agentskills.io">Agent Skill</a> interface
</p>
...
<p align="center">
  ...
  <a href="./SKILL.md"><img src="https://img.shields.io/badge/skill-0.8.16-0ea5e9?style=for-the-badge" alt="Skill version" /></a>
...
| A multi-slice field is about to start | Ask both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children; neither wrote the slice) at init / first wave. Never silent. Store on `done_when`. "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → `of close --checklist`. Opt-out is close, not one-role. After close, ask `of learn`. Self-praise is not review. Contrast still required. Not a new close gate. |
