...
| A multi-slice field is about to start | Ask both `--role adversary` and `--role verifier` (fresh-context review packet) at init / first wave. Never silent. Store on `done_when` / constraints. Run both at mission end; do not re-ask. After close, ask `of learn`. Self-praise is not review. Contrast still required. Not a new close gate. |
