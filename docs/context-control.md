...
| Second mission, same tree | `of new` → `.orderfield/fields/<id>/` + `.orderfield/ACTIVE` | Resume/status follow ACTIVE; roster if unmatched and no pointer (`choose`: epic vs patch); `--field` / `OF_FIELD`; `of fields` marks ACTIVE; not a product subfolder |
