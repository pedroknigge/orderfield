...
- Review-role consent default-proposes **both** `adversary` and `verifier` (two packs / two children; neither wrote the slice). Store `--done-when-mission`. Legal ask: "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → `of close --checklist`. Opt-out is close, not one-role. Exclusive-or menus fail `SkillEvaluatorPacket`. After close, ask `of learn` / `--promote`. Not a new close gate. No new CLI verb. Not a new regime.
