...
- Invent is a plan MD created after pin baseline, not an unpinned pre-existing file (`PlanIngress`; `#326`). `.orderfield/plan-baseline.json` on apply/pin. Shared monorepo trees no longer HOLD invent. Proof: `PlanIngressUnit` / `PlanIngressGate`.

## 0.8.25 — plan ingress fidelity (folder / chat / prompt → disk contract)

- Incoming plan is the disk contract: folder cites existing paths only; chat requires a durable capture; prompt promote is verbatim + `plan_source` sha pin (`PlanIngress`; `#324`). `plan_fidelity gap|invent` HOLDs pack. Paste without promote stays honest. No new verb. Proof: `PlanIngressUnit` / `PlanIngressGate` / `recovery/plan-ingress-promote`.
...
Folded Ready PRs `#295`–`#301` onto main after `#293`. Same 0.6 line. `#281` lands on this line (no VERSION bump).
...
Landed on main after the published v0.8.21 tag (`fa507f6`, #266) while still claiming 0.8.21 lockstep.

- Pack size-note claims write only after the packet is on disk. Proof: `SliceLintGate.test_failing_pack_does_not_claim_packet_written`. `#267`.
- PAIR `--verified-contract` without `--both-sides` refuses and does not stamp. Proof: `PairVerifiedContractRefuse`. `#268`.
...
`of learn --list` reads the protocol store when no ACTIVE/open field is bound. Two or more closed fields are not a `--field` refuse. Two or more open fields still die. Reuses `bind_active_field` allow-None + `list_learnings(None)`. Proof: `LearnListClosedFields`. Closes `#231`.
...
- `--surface {contract,internal}` after `--add` is no longer a silent no-op. `of spec --surface internal ID` reclassifies; WAL records REQUIREMENTS.json. `--surface` without `--add` or ID refuses. `--supersede` still drops the ID (coverage shrinks). `ContractSurface` still cannot hide. Contrast `next` / `--diff` teach the path. Proof: `RequirementSurfaceReclassify` / `SkillRequirementSurface`. No new CLI verb / schema / supervisor. Not a new regime.

## 0.8.11 — Host/worktree teardown after collect

- After collect or abandon, tear down opt-in of-worktrees (`of worktree remove --child-id`) and Orca Host panes (`orca worktree rm` / `terminal close --tab`). `of doctor` / close WARN when a recorded tree is orphaned vs a settled child. Prefer `--worktree current`. C-045. Proof: `DoctorWorktreeLeftover` / `SkillOrcaWorkerTeardown`. Not a process supervisor. Not a new regime.

## 0.8.10 — gc / retain survive a learning with phase

- `learning_applicable` used `PHASES` without importing it (`#205`). Empty-field gc stayed green because the guard only fires when a stored learning has `phase`. Import `PHASES` from `of.field`. Proof: `LearningApplicablePhase` / `EpisodicRetention.test_gc_and_retain_survive_field_learning_with_phase`. No new CLI verb / schema / supervisor. Not a new regime.

## 0.8.9 — next is INTEGRATE after a successful collect

- `next_legal_action` stayed on COLLECT after `of collect` returned `ok=N invalid=0 missing=0` because `integrated` is only `report.json` (`#204`). `CollectReady` reuses `session.last_cmd=collect` plus residual completeness. Before collect and after integrate stay COLLECT / NEXT-WAVE. Proof: `ResumeAfterIntegrate.test_next_is_integrate_after_successful_collect` / `SkillCollectNextIntegrate`. No new CLI verb / schema / supervisor. Not a new regime.

## 0.8.8 — issue --body-file accepts leader ISSUE-*.md

- `_issue_scratch_rel_ok` already gates canonical non-symlink scratch drafts. Leader HITL drafts live under `.orderfield/work/scratch/leader/`. A named draft (`ISSUE-of-issue-search.md`) was refused because depth-4 required exact `ISSUE.md` (`#201`). Leader now accepts any `.md` whose stem passes `_issue_id_ok` (same class as `issues/<slug>.md`). Child paths stay `ISSUE.md` / `issues/<slug>.md`. Proof: `IssueCli` / `SkillIssueBodyFileLeader`. No new CLI verb / schema / supervisor. Not a new regime.
...
- Cursor has no cheap/frontier `AdapterHints` alias (catalog: no frontier row). A consented `--model-tier` without `--model` refuses spawn, and pack when `ORDER.harness` is cursor. Named `--model` still passes. Claude still maps cheap→haiku / frontier→opus. Grok/agy/codex stay no-op. Do not invent a Cursor alias. Reuses `AdapterHints.spawn_model` / `spawn_flags`. Proof: `AdapterHintsCli` / `SkillCursorTierRefuse` / `recovery/cursor-tier-model`. No new CLI verb / schema / supervisor. Not a new regime.

## 0.8.5 — of issue --search lists open issues

- `of issue --search [QUERY]` lists open issues on `pedroknigge/orderfield` via the Issues list API and a local filter. Empty or omitted lists all open. `gh --search` (Search API) is not used — that path returned empty + exit 0 while `gh issue list --state open` showed rows (`#198`). Proof: `IssueSearchList` / `SkillIssueSearch`. No new CLI verb / schema / supervisor. Not a new regime.
...
- When SPEC / constraints cite plan docs (`docs/plans/…`), `of doctor` / close / integrate / amend print `docs_sync stale|pending|findings` if those files are older than last integrate (`PlanDocSync`). Mode A patch the named docs; Mode B write `work/scratch/leader/DOCS_SYNC.md` and ask to promote. Project findings are not chat. WARN, not FAIL, not a close gate. Reuses `RunbookPath.PATH_RE` / `AuditPressure`. Proof: `DoctorPlanDocSync` / `SkillPlanDocSync` / `recovery/plan-doc-sync`. No new CLI / schema / supervisor. Not a new regime. Cited on-disk plan `##` heading IDs without a packet print `plan_cover orphan` (`PlanCoverage`; #279 / #312). Init reads cited files/directories (`plan_ingest N`). SPEC / chat paste / `@folder` is not ingest. Constraint `plan_cover fail-closed` HOLDs close. Same advisory default. Proof: `recovery/plan-first-coverage` / `recovery/plan-ingest-paste`. Green collect on a mapped heading surgically writes the cited plan MD (`PlanWriteBack`; #313): checkbox / `Status:` / `Shipped:`. Fail-closed without OwnedWrite / close evidence. `docs_sync=done` only after plan bytes change or skip. Proof: `PlanWriteBackUnit` / `PlanWriteBackGate`.
...
**Stay-on-the-run** is the chosen loop. Pulse `STALE` means the packet is still the work: `of handoff` or `of spawn` on that same packet this turn. Do not unpack by default. Do not pack a sibling. Do not wait forever. `of pulse --watch` exits when idle (prints next; do not sleep); it is not a daemon and not a 5-minute kernel poll. A truly dead child is an explicit `of unpack`. The kernel never kills or auto-merges.
