...
- Review-role consent is a start-of-mission ask for **both** `adversary` and `verifier`. Store on `--done-when-mission` / `--constraints-add`. Run both at the configured end; do not re-ask a XOR menu. After close, ask `of learn` / `--promote`. Proof: `SkillEvaluatorPacket`. Not a new close gate. No new CLI verb. Not a new regime.
