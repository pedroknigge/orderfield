...
| `spawn` | After `of spawn` / generic handoff spawn path | `adapter`, `child_id`, `ok`, `outcome` (`ok` \| `done_without_residual` \| `nonzero_exit` \| `timeout` \| `missing_binary` \| `error` \| `dry_run`); `ok` is false when a schema-valid residual is missing (`done_without_residual`); `exit` on ok/nonzero_exit/done_without_residual, `timeout_s` on timeout, `mode: handoff` on the generic path; `operator_actions` (`yolo` and/or `inherit`) when those audited operator actions are set |
...
| `close` | After `of close` | write path: `rev`, `spec_hash`, `done_when_closed`, `ok`; `parent` when ACTIVE returned; `written=true`, `residual_empty`; `scratch_wiped` count after the stamp. `--checklist`: `checklist=true`, `written=false`, `ok`, `contrast`, `residual`, `in_flight`, `in_flight_ids`, `evaluator`, `evaluator_ids`, `evaluator_roles`, `next` — no stamp, no wipe. `evaluator` is ask-only status, not a close gate |
...
| `doctor` | After `of doctor` | `ok` (field/kernel); `ok_field`; `ok_skills` (skill SKEW is advisory); `leftover_worktrees` (orphaned `of worktree` count vs settled children; advisory WARN, not Orca poll); `audit_over` (tree audit OVER or fat scratch; advisory WARN, not FAIL); optional `ram_total_gb` / `ram_avail_gb` / `suggested_band` (`HostRam`; wave budget, not a spawn cap) |
