...
| `doctor` | After `of doctor` | `ok` (field/kernel); `ok_field`; `ok_skills` (skill SKEW is advisory); `leftover_worktrees` (orphaned `of worktree` count vs settled children; advisory WARN, not Orca poll); `audit_over` (tree audit OVER or fat scratch; advisory WARN, not FAIL) |
