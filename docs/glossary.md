...
**VERIFIED_CONTRACT** closes a public surface named in SPEC (CLI, HTTP, file format, exit code). Pair-shaped requirements need both sides (`of spec --verified-contract ID --both-sides`). Webhook HMAC signature + replay is PAIR (`WebhookPair`: accept valid and reject replay/bad signature). `of close` stays blocked while a public-surface ID is only internally verified.
