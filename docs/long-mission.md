...
of resume               # execute printed next same turn; report is not a stop (`DriveAfterIntegrate`)
...
of next-wave        # do not --recompute the prior wave; ordinary next-wave is not a consent ask
of wave list
of wave show
of status --json
```
