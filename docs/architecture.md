...
**Status:** Active · **Stack:** Python 3.11+ stdlib · **Version:** `0.7.41` — see [`VERSION`](../VERSION)
...
| `cmd_spec` / `cmd_spec_diff` / `cmd_contrast` / `cmd_close` | Binding-requirement ledger (index over SPEC: `origin` + line range), SPEC↔ORDER omissions, public-surface close gate (`VERIFIED_CONTRACT`; pair `--both-sides`). `of close --checklist` is contrast + residual empty (`CloseChecklist`); write path refuses residual MISSING |
...
| `argv_preview` / `redact_text` / `ArgvRedact` | Secrets and escalated approval flags stripped from spawn previews and logs; long `--output-schema` / path tokens keep a basename so a deep skill root still names `residual.codex.schema.json` |
