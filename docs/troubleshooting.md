...
**Recover:** Install a harness CLI onto PATH if you need headless spawn. `of init` if there is no field. Do not treat a PATH hit as logged-in. Skill SKEW alone is not a broken field — `bash install.sh --global` (or copy this checkout) into the dest that printed skill `SKEW`. Audit OVER / fat scratch: `of gc --audit` then shrink / `--keep-field` / `--drop-field` before `of close`. For leftover root ORDER.json SKEW: `of migrate` (archives the stub). For ACTIVE stub/dangling: `of fields` then `--field` / `of new`. For aged **open-field** packs: `of resume` / `of handoff` (not a daemon). For **open-field** `order_rev` stale packs: `of next-wave`. For sibling fields without CLOSE: `of fields` then `of close` or `of gc --archive-field` (closed trail). Closed-field historical packs are informational — leave the audit trail.
