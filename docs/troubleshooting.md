...
**Recover:** Restore the canonical inputs if they were changed accidentally. If the change is intentional and reviewed, run `of integrate --wave N --recompute` (plus the same `--partial` / `--apply` choices). The replacement points to a content-addressed integration record and retains history. `of resume` / `of status` print `INTEGRATE --RECOMPUTE` (not `NEXT-WAVE`) when `report.json` exists but the covering digest drifted. Spawn finalization of `session_id` / `denied_actions` after integrate is not drift (`IntegrationDigest`). `done_when_closed` is part of that digest (#49): after `of patch --done-when-closed`, a plain replay is refused; `--recompute` selects `phase` instead of replaying hold.

## Phase or next-wave refused

The transition error names the missing proof: in-flight children, no integration report (when packets exist), changed integration digest (`of integrate --wave N --recompute`), unclosed phase, non-sequential target, wrong report regime, or no ORDER revision after escalation. An empty wave has nothing to integrate — `of phase <next>` after `--done-when-closed` does not need `--force`. Close that condition and retry. `phase --force --reason "…"` is the audited break-glass path for phase movement; `next-wave` has no force flag. `--force` to `deliver` still requires `of close` / coverage / matching `spec_hash`.
...
**Meaning:** Doctor prints kernel-verifiable local checks in one pass: Python, kernel VERSION, writable field + scratch, public schemas, whether `.orderfield/field.lock` is acquirable, skill VERSION skew, ACTIVE pointer/stub skew, leftover root ORDER.json migrate-required, sibling fields open without CLOSE, stale packs, and tree-audit size. When a newer GitHub release exists than the installed VERSION, doctor also prints an `update` ask at most once a day (`UpdateAsk`) and, on a TTY, prompts `[y/N]`. On yes: `install.sh --global --from-release` (release tar.gz + SHA256SUMS). Never a silent auto-update. `OF_NO_UPDATE_CHECK=1` disables. Silent offline. It scans known HOME dests that already exist (`.agents`, `.claude`, `.codex`, `.cursor`, `.opencode`, `.grok`, `.gemini/…`) and compares each readable `VERSION` / `SKILL.md` metadata to this checkout. Missing dests are silent — doctor does not invent installs. Skill VERSION `SKEW` is advisory: `doctor WARN` / exit 0 and `ok_skills=false` on the doctor event. Tree audit `OVER` or a fat scratch child (8 MiB) is the same WARN (`AuditPressure`); `of close` / `--checklist` print the note too — not a close gate. `--keep-field` still silences. Cited plan docs (`docs/plans/…` in SPEC / constraints) that are older than last integrate print `docs_sync stale` (`PlanDocSync`); a fresh `work/scratch/leader/DOCS_SYNC.md` is `pending` (ask to promote). Open project findings without a dump print `findings`. A cited on-disk plan heading ID with no packed child prints `plan_cover orphan` (`PlanCoverage`; `#279` / `#312`). Init prints `plan_ingest N headings` when the cited file exists. Heading IDs only in SPEC / chat paste / `@folder` print `plan_cover paste` and do not count as ingested. Constraint `plan_cover fail-closed` HOLDs `of close`. WARN / exit 0 default. Not FAIL. Not a CMS. A leftover root `ORDER.json` after nested homes exist prints `migrate required` plus `SKEW` and FAIL (exit 2) — run `of migrate` (never a silent delete). A dangling `.orderfield/ACTIVE`, an in-flight `packed_at` older than the 7-day SLA on an **open** field, or an **open-field** `order_rev` stale pack print `SKEW` / `packed_age` and FAIL (exit 2). Two or more sibling homes still open with no `CLOSE.json` print `open N fields no CLOSE` as hygiene WARN (not FAIL). Closed-field historical packs (retained `order_rev` / `packed_age` older than the final ORDER) print field id + wave as informational and do not FAIL. Do not rewrite `CLOSE.json` or retained packets to green the current field. Adapter lines show PATH + best-effort `--version`. **PATH is not auth, credentials, session authority, or readiness** — doctor prints `auth=not-verified` / `ready=not-verified` on purpose. Kernel verifies PATH/argv/residual; the harness promises approval/auth/ready. Doctor does not write ACTIVE.

**Recover:** Install a harness CLI onto PATH if you need headless spawn. `of init` if there is no field. Do not treat a PATH hit as logged-in. Skill SKEW alone is not a broken field — `bash install.sh --global` (or copy this checkout) into the dest that printed skill `SKEW`. Audit OVER / fat scratch: `of gc --audit` then shrink / `--keep-field` / `--drop-field` before `of close`. Successful `of close` then wipes `work/scratch` and wave logs/spawns/prompts (`ClosedScratch`); `--checklist` does not. For leftover root ORDER.json SKEW: `of migrate` (archives the stub). For ACTIVE stub/dangling: `of fields` then `--field` / `of new`. For aged **open-field** packs: `of resume` / `of handoff` (not a daemon). For **open-field** `order_rev` stale packs: `of next-wave`. For sibling fields without CLOSE: `of fields` then `of close` or `of gc --archive-field` (closed trail). Closed-field historical packs are informational — leave the audit trail.
