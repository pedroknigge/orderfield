...
The kernel grew from 0.3.2 through 0.7.2. The physics stayed a method. No new regime.
...
**Status:** Introduced by `0.3.2`, current in `0.7.2` · **Code:** [`scripts/of.py`](../../../scripts/of.py), [`scripts/of/`](../../../scripts/of/), [`scripts/of_adapters.py`](../../../scripts/of_adapters.py), [`schemas/`](../../../schemas/)
...
- `of retain` (read-only) / `of gc` apply episodic retention: keep still-useful residuals and **protocol** learnings, drop inapplicable **field** learnings. The plan action `dump` is **permanent unlink** of logs and wave history older than 30 days (`Path.unlink` / `rmtree`), not an export. Backup is operator-owned. Protocol is not unlinked at 30 days. Never copy transcripts. WAL crash consistency is not a restorable dump.
