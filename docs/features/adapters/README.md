...
**Status:** Introduced by `0.3.2`, current in `0.8.22` · **Code:** [`scripts/of_adapters.py`](../../../scripts/of_adapters.py) (imported by [`scripts/of.py`](../../../scripts/of.py))
...
`OF_TRUST` is authoritative for every adapter. Residual packs default to the write-floor (`auto-edit` / `WriteFloor`) for claude/codex/agy/qwen. cursor/grok/opencode/orca/generic WARN + named next (no invented flags). Explicit `conservative` is the opt-out. `yolo` is the only bypass. `yolo` and `OF_SPAWN_ENV=inherit` are audited operator actions (`OperatorAction`), not silent defaults. Host allow-lists are advisory only. agy/grok isolate host global MCP by default (`HostMcp`); `OF_SPAWN_MCP=inherit` opts in. Full table: [references/adapters.md](../../../references/adapters.md#trust-profiles-of_trust).

- **grok:** `-p`; documented `--output-format streaming-json` before `-p` (`StreamJson`); residual extract from stdout; `OF_TRUST=plan` adds `--sandbox read-only`; `--always-approve` only under `OF_TRUST=yolo`; consented named `--model` before `-p`. Host global MCP is isolated by default (`HostMcp`; same path as agy). Spawn metadata finalized on exit/timeout/missing binary. `--json-schema` omit.
...
- **agy:** flags before `-p`; `--json-schema` to `residual.codex.schema.json` (same Codex file; `OutputSchema`); Codex-null optionals are omit (`CodexNullOmit`); invalid stdout extract names `$.path`. `OF_TRUST=plan` prepends `--mode plan`; `--dangerously-skip-permissions` only under `OF_TRUST=yolo`; consented named `--model` before `-p`. Conservative spawn copies nonempty JSON `denied_actions` into `residual.denied_actions` (`AgyDeniedActions`); missing/empty is not approval; `yolo` does not copy. Host global MCP is isolated by default (`HostMcp`); `OF_SPAWN_MCP=inherit` opts in (agy has no `--no-mcp`). Skills: Global `~/.gemini/antigravity-cli/skills`; Shared `~/.gemini/skills`; `~/.gemini/config/skills` optional legacy. Quoted `description` / `compatibility` frontmatter so agy discovers the skill.
...
- **orca:** `of spawn --adapter orca` is one-shot `task-create` (substrate). Interactive `worker-start` is a start↔stop/release pair taught by the skill — after collect or abandon, `worker-stop` then `worker-release`; retain only if asked; `of worktree remove` if add was used; Host panes: `orca worktree rm` / `terminal close --tab`. Kernel does not poll or kill Orca processes.
- **generic / `OF_AGENT`:** shell-quoted argv (`GenericAgent` / `shlex.split`). Quote paths with spaces. Dry-run prints `shlex.join` of the real list so a space path stays one token. No `OF_TRUST` translation. Explicit `--adapter generic` handoff if `OF_AGENT` is unset. Implicit spawn when detect is present:none refuses (`SpawnAdapterMissing`) — not silent handoff-to-self. A second pack in that wave WARNs. Cloud hosts with no CLI are single-session only.
- **model hints:** optional. The `/of` skill consults [docs/model-catalog.md](../../model-catalog.md) then must propose cheap vs frontier in chat before a multi-role pack. After consent (`of patch --model-hints` or pack `--model-tier` / `--model`), spawn may pass `--model` for claude / codex / cursor / grok / agy. Claude maps cheap→haiku, frontier→opus. Cursor has no cheap/frontier alias (catalog: no frontier row); tier-only **refuses**. Grok and agy pass a named `--model` before `-p`; tier-only is no-op (no invented aliases). Orca `task-create` has no `--model` (disk hint only). Qwen, opencode, and generic stay no-op. No consent → argv unchanged. `AdapterHints` in `scripts/of_adapters.py`. Advisory catalog is not a router. Not a silent switch.
