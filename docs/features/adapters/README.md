...
**Status:** Introduced by `0.3.2`, current in `0.7.49` · **Code:** [`scripts/of_adapters.py`](../../../scripts/of_adapters.py) (imported by [`scripts/of.py`](../../../scripts/of.py))
...
- **model hints:** optional. After consent (`of patch --model-hints` or pack `--model-tier` / `--model`), spawn may pass `--model` for claude / codex / cursor. Claude maps cheap→haiku, frontier→opus. Orca `task-create` has no `--model` (disk hint only). Qwen and the rest stay no-op. No consent → argv unchanged. `AdapterHints` in `scripts/of_adapters.py`. Not a catalog.
