...
**Status:** Introduced by `0.3.2`, current in `0.7.67` · **Code:** [`scripts/of_adapters.py`](../../../scripts/of_adapters.py) (imported by [`scripts/of.py`](../../../scripts/of.py))
...
- **claude / cursor:** `--output-format stream-json` so spawn can append harness events to the same `scratch/<id>/PULSE` (`StreamJson` / `PulseProgress`). Residual extract from stdout stays. Cursor `OF_TRUST=plan` adds `--mode plan`. `--resume ID` only when `residual.session_id` is already set (`AdapterResume`); never `--continue`. Not a supervisor.
