...
| SCOPE-COST | DEP-COST-CONTROLS-MISSING | `budget.tokens`, `thresholds.local_budget_pct`, and `scale_up` stay **reserved**. The kernel does not measure paid usage. Inventing estimates, threshold alerts, or ceilings without telemetry would be a fake budget. Optional `residual.usage` is child-copied provenance when the harness reported tokens/model — not a ledger and not compared to `budget.tokens`. | Spawn prints `of: cost: harness paid usage is not measured; this is not a budget` (`COST_DISCLAIMER` / JSON `kind=cost_unmeasured`). `of pack` writes `tokens=0`; `--tokens N` for N>0 dies (`kind=reserved`). `EfficiencySignal` may propose a model-tier **ask** from residual quality × optional usage; it does not enforce a ceiling or switch a model. Only `budget.seconds` is enforced (spawn wall-clock); `of spawn --timeout` must match or be omitted. DEP-COST-CONTROLS-MISSING is Partial-by-design while the disclaimer exists. Do not implement COST ceilings. |
| SCOPE-REVIEW | PROCESS-REVIEW-ENFORCEMENT / REVIEW-001 | Independent GitHub approval is **human merge practice**, not a kernel command. Do not add `of merge`. Do not lower protection. Do not self-approve. The in-field fresh-context review packet (`adversary`/`verifier`, ask first) is a different plane — not REVIEW-001. | Branch protection is already configured: `required_approving_review_count >= 1`, dismiss stale, enforce admins, five checks, force-push and deletion disabled ([CONTRIBUTING.md](../../CONTRIBUTING.md)). REVIEW-001 stays **unproven** until a non-author collaborator reviews (C-080 Partial). PROCESS-REVIEW-ENFORCEMENT is process, not code. |
...
| SCOPE-GODSPLIT | (form) | Splitting `scripts/of/field.py` WAL/view/learning, parser registration, and spec eval fixtures is allowed as **focused form PRs** under the existing suite. Not a product feature. Mixing a god-module split into LEARN/WAL behavior PRs is theater. **Shipped:** named extractions landed ([PR #59](https://github.com/pedroknigge/orderfield/pull/59), polish [PR #103](https://github.com/pedroknigge/orderfield/pull/103)). Do not reopen this ID because `field.py` is still ~4k lines after later feature cuts. Inventing `field_lock.py` / `field_pulse.py` / `field_migrate.py` is theater. | WAL/view → `scripts/of/wal.py`, learnings → `scripts/of/learn.py`, retention/gc → `scripts/of/retain.py`. `field.py` re-exports **public** names and routes through `FieldWal` / `FieldLearnings` / `FieldRetain`. Private `_` helpers stay on the owner module. Parser registration and eval fixtures already live in `scripts/of/cli/`. Remaining `field.py` is the field I/O owner (paths, lock, spawn registry, schemas, migrate, pulse, session, redact). Public CLI and protocol stay. |
