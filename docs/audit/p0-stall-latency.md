...
| Cut | Issue | Title |
|---|---|---|
| parent | *(pending)* | P0: stall/latency — child dispatch + silent death (audit 2026-09-17) |
| P0-1 | *(pending)* | spawn success prints exit=0 and stops; reuse DriveAfterIntegrate next=COLLECT |
| P0-2 | *(pending)* | dead started-only spawn says Wait for it / HOLD forever |
| P0-3 | *(pending)* | pulse --watch never exits when in_flight=0 |
| P1-1 | *(pending)* | SKILL pre-pack consults add wall-clock before first child |
| P1-4 | *(pending)* | pulse STALE while spawn pid is still alive |
