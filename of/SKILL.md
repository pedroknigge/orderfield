...
Campo then Orden: `of config` audits installed harness CLIs (PATH≠auth) and their catalog models. `of config set --contestant MODEL EFFORT` (repeat, N>=2) stores peers `{model, effort}` — not a role and not a rank. Empty until set. Example data lives in the sibling README and is not a preference. `of init --campo` opens the arena on this branch. Contestant #1 is the session running `of`, a peer, not a parent. The rest of the roster is a headless stub in `campo/spawns.json` (not launched here). Peers elect the leader; the host does not appoint one. `of campo settle` pins leader, crew, and ORDER. Same branch + commit = shared context: commit the proposal, ballot, code, or residual note; others refresh from that commit. No worktree. No merge-packet. Owned paths or turn-taking when one path would collide. Then pack. Follow the sibling.
