...
Leader HITL `--body-file` is `.orderfield/work/scratch/leader/ISSUE.md` (or the child's existing `ISSUE.md` / `issues/<slug>.md`). Refuse names `.orderfield/work/scratch/<child_id>/`. `of issue --search [QUERY]` lists open issues (empty or omitted = all open; a query filters that list). Follow the sibling.
