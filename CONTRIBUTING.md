...
Stdlib. Protected `main`. No pip by accident.
...
| Leader / child doctrine | `SKILL.md`, `references/skill-appendix.md`, `CHILD.md`, `AGENTS.md` | Short core + appendix; protocol, not a new regime |
...
| Claims honesty gate | shipped 0.7.50 | `python3 docs/audit/check-claims.py` (`ClaimsHonesty`) in `validate-skill.sh`. Unique C-IDs; advertised truth score matches the table and stays ≤98%; critical Contradicted fails; theater on SKILL / `/of` / README dies. `ClaimsHonestyGate`. |
...
| `field.py` WAL/view/learning split (SCOPE-GODSPLIT) | shipped | Named extractions landed [PR #59](https://github.com/pedroknigge/orderfield/pull/59) + polish [PR #103](https://github.com/pedroknigge/orderfield/pull/103): `of.wal` / `of.learn` / `of.retain`; `field.py` re-exports public names and talks through `FieldWal` / `FieldLearnings` / `FieldRetain`. Remaining `field.py` is the field I/O owner (bind, lock, roster, pulse, migrate, worktree, schemas, session). Later feature growth is not an open god-split. Do not invent `field_lock.py` / `field_pulse.py` under this ID. Not a product release. |
