...
Classic install is tag-pinned and SHA-256 verified. Do not pipe unsigned `main`. Unpinned `npx skills add …` is **not** the trusted path (SCOPE-NPX). README portada is glance only (pin + `--from-release` + PATH); this page keeps the full SHA-256 ritual.
