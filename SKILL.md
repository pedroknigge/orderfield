...
| status=done residual | `artifact_sha:` + `rollback:` (`CloseEvidence`). Implementer/`--owns-path` hashes owned product or published artifact, not scratch. Empty `--owns-path` is `owned_write_missing`. Mtime-only is not a write (`OwnedWrite`). Do not trust status. Not `of prove` |
