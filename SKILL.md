...
| any residual MISSING (`running`) | `of status` / `of resume` already print the live `PULSE` (child heartbeat + spawn stream-json / grok `streaming-json` on the same scratch) + a `speak` line — quote one `PULSE` line to the user and do not claim done; no manual `of pulse` (`--watch` exits when idle + prints next; not the product path). A leftover residual from a prior collect refuse does not hide a started-only re-spawn — stay `running` + speak until that spawn settles |
...
| status/resume says `PACKED` / `next SPAWN` | no spawn record; spawn only if detect present. present:none → HOLD. `of handoff --packet` ≠ spawned child / dogfood. Do not HOLD as if ALIVE. `spawned` counts spawn metadata, not packs |
| `OF_TRUST=yolo` / `OF_SPAWN_ENV=inherit` | **must ask**. Audited operator actions, not silent defaults. Spawn prints `operator action`. Never invent. yolo is never implied. inherit forwards the parent env. Conservative + allowlist stay defaults. `OF_TRUST=plan` → `--mode plan` / grok `--sandbox read-only`. agy/grok isolate host MCP (`OF_SPAWN_MCP=inherit` opts in — ask). Pulse `cpu=` on `no writes yet`. Table: appendix + adapters.md |
