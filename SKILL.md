...
| resume next HOLD + started-only pid gone | `of spawn --force-spawn` on the same packet — do not pack. Live pid still refuses |
...
**Auto-revival.** Open field (`spec_closed` false): every turn **`of resume` first**, then **execute printed `next` same turn** — including after collect+integrate. A status report is not a stop; do not wait for ok/pulse. HOLD = continue existing packets, not invent a consent ask. HOLD detail naming `SPAWN --FORCE` means `of spawn --force-spawn` on the same packet (started-only pid gone); do not pack. Ordinary next-wave/pack is not the adversary/harness/model ask. Pause only on explicit `pause` / `stop` / `wait on the field` / `cancel the mission` / `of init --force`. Resume-without-`next` is a broken run. Clone/checkout + HOME dest skill is the same.
