...
| Campo then Orden (hard gate: roster or `--orden-only`; canonical `of new --campo`) | `of config set --contestant MODEL EFFORT` once. `of new --campo` launches and pins. Host does not appoint. Same branch + commit = shared context. c1=session. |
...
| build/test log above floor | archive exact bytes; cite `evidence_receipt:` (hash/exit/quotes/paths/size/command_id). Cited receipt must ACCEPT (`EvidenceReceipt`). Tamper ≠ green. File read/search bypass. Wave-end verifier prefers receipts + published artifacts over narrative |
...
| residual / `OF_TRUST` | write-floor default. unsupported WARN + next. `conservative` opt-out. `OF_TRUST=yolo` / `OF_SPAWN_ENV=inherit` **must ask** — not silent defaults; operator action; never invent. `plan` → `--mode plan` / grok `--sandbox read-only`. agy/grok isolate MCP (`OF_SPAWN_MCP=inherit`). Pulse `cpu=` on `no writes yet`. |
...
Run `of` on PATH (`~/.local/bin/of`) or `python3 <skill>/scripts/of.py`.
