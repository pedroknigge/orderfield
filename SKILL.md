---
name: orderfield
description: "v0.7.66 — Disk-backed plan that survives chat, a token cut, and a model switch. Use when the user invokes /orderfield or /of, an existing field must be resumed, or a genuine multi-slice wave needs a disk brief. While any child flies, of status/resume print running + live PULSE + a speak line (stream-json feeds same PULSE); quote it, never claim done (no of pulse). Before pack: consult model-catalog, then propose cheap vs frontier and ask same-harness vs multi-harness mix (consent; detect present/missing PATH≠auth). Consented spawn --model includes grok/agy (named; no aliases). Spawn --resume ID only when residual.session_id already set; never invent; never --continue. agy --json-schema reuses residual.codex; Claude omit. Status/resume may propose uptier/downtier — ask, never silent switch. Install: mortal-install.sh then of doctor. Claims ≤98% honest (check-claims.py). Gaps: of contrast --diff. Daily update ask. Do not trigger for a harness name alone. Unknown harnesses use generic."
license: MIT
...
| any residual MISSING (`running`) | `of status` / `of resume` already print the live `PULSE` (child heartbeat + spawn stream-json on the same file) + a `speak` line — quote one `PULSE` line to the user and do not claim done; no manual `of pulse` |
