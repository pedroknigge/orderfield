...
`of init --force` replaces **this** field: old wave dirs are archived to `waves-archived-<old id>/` so `state.wave` stays true (no silent jump from wave 1 to wave N later) and stale packets never shadow the new mission. To keep the current field and start another in the same tree: `of new --mission "…"`. A phase of this epic: `of new --parent --mission "…"`. First `of init` still writes legacy `.orderfield/ORDER.json`; the first `of new` promotes it under `fields/<id>/`.
