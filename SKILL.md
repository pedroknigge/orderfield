...
| resume next `UNPACK --FORCE` / spec rev-stale note | `of unpack --force --child-id <id>` — ORDER.rev staled every live packet (scratch kept). Or `of next-wave`. Do not spawn. `of spec --add` / `--amend` warns that the bump stales N packet(s) |
