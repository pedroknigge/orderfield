...
| init / first wave plan | **must ask** both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children; neither wrote the slice). Never silent. Store `--done-when-mission`. "Run adversary + verifier before close?" Yes → pack+spawn both. No → contrast → `of close --checklist`. Opt-out is close, not one-role. After close: `of learn`. Self-praise is not review. Not a new close gate. Not after ordinary integrate. |
