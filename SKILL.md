...
ORDER, packets, residuals, and regime decisions live on disk. Use it when a kernel, a product, or a multi-slice build needs exclusive owners, a SPEC that survives compaction, and `of contrast` before close.
...
| init / first wave plan | **must ask** once (store `--done-when-mission`): "At the end, run fresh-context adversary + verifier (both)?" Never silent. Yes → pack+spawn both `--role adversary` and `--role verifier` (fresh-context review packet; two packs / two children). No → contrast → `of close --checklist`. After close: `of learn` / `--list`, not the review-role ask. Not after ordinary integrate. |
...
| `of doctor` prints FAIL | leftover root ORDER.json SKEW needs `of migrate` (FAIL). Sibling fields without CLOSE are hygiene (WARN). Closed-field historical packs are informational |
...
| `OF_TRUST=yolo` / `OF_SPAWN_ENV=inherit` | **must ask**. Audited operator actions, not silent defaults. Spawn prints `operator action`. Never invent. `OF_TRUST=plan` → `--mode plan` / grok `--sandbox read-only`. agy/grok isolate host MCP (`OF_SPAWN_MCP=inherit` opts in). Pulse `cpu=` on `no writes yet` |
