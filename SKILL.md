...
| Campo then Orden | `of config set --model A --model B --effort medium`, then `of init --campo`. Peers ballot; `of campo settle` pins leader + ORDER. Host does not appoint the leader. Same branch. Then pack. |
