...
| init / first wave plan | **must ask** both `--role adversary` and `--role verifier` (fresh-context review packet) for mission end. Never silent. Store `--done-when-mission` / `--constraints-add`. End: run both; do not re-ask. Stored no → contrast → `of close --checklist`. After close: `of learn` / `--promote`. Self-praise is not review. Not a new close gate. Not after ordinary integrate. |
| after wave, before close | leftover field `of learn` + reportable errors — **must ask** `--protocol`/`--promote` OR owned `docs/plans` OR keep/discard; defects → `of issue` HITL and/or PlanDocSync A/B. Not auto-promote. Not a new close gate. Not after ordinary integrate. Not `of merge`. |
