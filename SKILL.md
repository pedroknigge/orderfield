...
| status/resume says `PACKED` / `next SPAWN` | no spawn record / scratch; `of spawn` if detect present. present:none → HOLD. `of handoff --packet` ≠ spawned child / dogfood. Do not HOLD as if ALIVE. `spawned`=spawn meta |
