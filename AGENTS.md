...
| 0.7.81 done residual close evidence requires artifact SHA + rollback command (`CloseEvidence`; collect refuses captions / mismatch; implementer/owns-path hashes product not scratch; `#286`) | documented |
...
| 0.8.20 OwnedWrite (`#251`; empty owns-path + implementer is `owned_write_missing`; mtime-only is not a write; `#286`); EscalateUnblock (`#254`); mid-flight `of patch` refuse (`#253`); never chain pack|spawn (`#255`); LiveQuietStuck HITL HOLD (`#256`). Landed on main after the v0.8.19 tag | documented |
...
| 0.8.22 pack size-note honesty (`#267`); PAIR `--verified-contract` refuse without `--both-sides` (`#268`); agy/grok isolate host MCP + pulse `cpu=` (`HostMcp`; `#269`); present:none HOLD / implicit spawn refuse (`SpawnAdapterMissing`; `#273`); pulse names packed-never-spawned (`InFlightSignal`; `#274`); residual write-floor default (`WriteFloor`; `#294`). Landed on main after the v0.8.21 tag | documented |
| 0.8.22 plan-first ORDER coverage (`PlanCoverage`; `#279` / epic `#278`): cited mega-plan / SPEC heading IDs without a packet print `plan_cover orphan`; high effort on ORDER, medium slices; not a close gate | documented |
