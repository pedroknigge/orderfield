...
| 0.8.15 collect unexpected keys name the legal home (`SchemaHomeHint`; `#215`); `validate_schema` subset honesty (`anyOf` / `maxLength` / `patternProperties`; `SchemaSubsetHonesty`; C-030); successful `of close` wipes `work/scratch` and wave logs/spawns/prompts (`ClosedScratch`; closed-ephemeral; C-084); spawn claim lock + `MUTATING_COMMANDS` honesty (`SpawnRecord.claim_started` / `mutating_commands_prose()`; `SpawnLockRace` / `MutatingCommandsHonesty`); HITL `of issue` lives in `cli/issue_cmd.py` (not `ops`); same verbs (`#219` second cut) | documented |
