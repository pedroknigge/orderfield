...
| 0.8.22 pack size-note honesty (`#267`); PAIR `--verified-contract` refuse without `--both-sides` (`#268`); agy/grok isolate host MCP + pulse `cpu=` (`HostMcp`; `#269`); present:none HOLD / implicit spawn refuse (`SpawnAdapterMissing`; `#273`); pulse names packed-never-spawned (`InFlightSignal`; `#274`); residual schema names taught `docs_sync` (`#292`). Landed on main after the v0.8.21 tag | documented |
