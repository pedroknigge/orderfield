...
| 0.7.85 fresh-context evaluator packet (`EvaluatorPacket`; ask both adversary+verifier at init; store; run at end; never silent; not a close gate; C-080 stays Partial) | documented |
...
| 0.8.16 `validate_schema` subset honesty (`anyOf` / `maxLength` / `patternProperties`; `SchemaSubsetHonesty`; C-030; `#223`); successful `of close` wipes `work/scratch` and wave logs/spawns/prompts (`ClosedScratch`; closed-ephemeral; C-084; `#224`); spawn claim lock + `MUTATING_COMMANDS` honesty (`SpawnRecord.claim_started` / `mutating_commands_prose()`; `SpawnLockRace` / `MutatingCommandsHonesty`; `#225`); HITL `of issue` lives in `cli/issue_cmd.py` (not `ops`; same verbs; `#226`; Relates `#219`). Landed on main after the v0.8.15 tag. Docs/skill: published-artifact check before FACTIBLE / close (`SkillArtifactProve`; CUMPLE-with-bed-overlap fails; no `of prove`); ask both adversary+verifier at init, store, run at end (`SkillEvaluatorPacket`; no XOR close menu) | documented |
