...
| 0.8.15 collect unexpected keys name the legal home (`SchemaHomeHint`; `#215`); `validate_schema` subset honesty (`anyOf` / `maxLength` / `patternProperties`; `SchemaSubsetHonesty`; C-030); successful `of close` wipes `work/scratch` and wave logs/spawns/prompts (`ClosedScratch`; closed-ephemeral; C-084) | documented |
