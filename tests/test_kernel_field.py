...
class ProtocolLearnings(unittest.TestCase):
    ...
    def test_forget_and_refuse_dump(self) -> None:
        ...
        stored = run_of(self.tmp, "learn", huge)
        self.assertEqual(stored.returncode, 0, stored.stderr)
        self.assertIn("of: note —", stored.stderr)
        self.assertIn("advisory", stored.stderr)
        self.assertIn("work/scratch/leader", stored.stderr)
        self.assertIn("still stored", stored.stderr)
        listed_huge = run_of(self.tmp, "learn", "--list")
        self.assertEqual(listed_huge.returncode, 0, listed_huge.stderr)
        self.assertIn(huge, listed_huge.stdout)
        dump = "\n".join(["line"] * (of.LEARNING_MAX_LINES + 1))
        refused = run_of(self.tmp, "learn", dump)
        self.assertNotEqual(refused.returncode, 0)
        self.assertIn("refuse dumps", refused.stderr)
        self.assertIn("lines", refused.stderr)
        self.assertNotIn("line line", run_of(self.tmp, "learn", "--list").stdout)
...
class CheckpointHandoffStayOnRun(unittest.TestCase):
    ...
    def test_next_legal_action_handoff(self) -> None:
        ...
        idle = {"wave": 1, "children_spawned": 1, "spawn_blocked": False}
        landed = [{"child_id": "c1"}]
        self.assertEqual(
        ...
        self.assertEqual(
            of.next_legal_action(
                idle, [], landed, integrated=True, covering=False
            ),
            "integrate --recompute",
        )
        self.assertEqual(
